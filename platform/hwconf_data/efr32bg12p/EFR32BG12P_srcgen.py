...
# get module mapping
from efr32bg12p.modules.PIN.PIN_Class import *
import efr32bg12p.PythonSnippet.ExporterModel as ExporterModel
import efr32bg12p.PythonSnippet.Metadata as Metadata
import efr32bg12p.halconfig.halconfig_dependency as dep

from efr32bg12p.PythonSnippet.CodeGen import *
from efr32bg12p.PythonSnippet.StudioProject import *

import efr32bg12p.modules.ANTDIV.ANTDIV_behavior as ANTDIV_behavior
import efr32bg12p.modules.BATTERYMON.BATTERYMON_behavior as BATTERYMON_behavior
import efr32bg12p.modules.BTL_BUTTON.BTL_BUTTON_behavior as BTL_BUTTON_behavior
import efr32bg12p.modules.BULBPWM.BULBPWM_behavior as BULBPWM_behavior
import efr32bg12p.modules.BULBPWM_COLOR.BULBPWM_COLOR_behavior as BULBPWM_COLOR_behavior
import efr32bg12p.modules.BUTTON.BUTTON_behavior as BUTTON_behavior
import efr32bg12p.modules.CMU.CMU_behavior as CMU_behavior
import efr32bg12p.modules.COEX.COEX_behavior as COEX_behavior
import efr32bg12p.modules.CS5463.CS5463_behavior as CS5463_behavior
import efr32bg12p.modules.DCDC.DCDC_behavior as DCDC_behavior
import efr32bg12p.modules.EMU.EMU_behavior as EMU_behavior
import efr32bg12p.modules.EXTFLASH.EXTFLASH_behavior as EXTFLASH_behavior
import efr32bg12p.modules.EZRADIOPRO.EZRADIOPRO_behavior as EZRADIOPRO_behavior
import efr32bg12p.modules.GPIO.GPIO_behavior as GPIO_behavior
import efr32bg12p.modules.I2C0.I2C_behavior as I2C_behavior
import efr32bg12p.modules.I2C1.I2C_behavior as I2C_behavior
import efr32bg12p.modules.I2CSENSOR.I2CSENSOR_behavior as I2CSENSOR_behavior
import efr32bg12p.modules.IOEXP.IOEXP_behavior as IOEXP_behavior
import efr32bg12p.modules.LED.LED_behavior as LED_behavior
import efr32bg12p.modules.LEUART0.LEUART_behavior as LEUART_behavior
import efr32bg12p.modules.LNA.LNA_behavior as LNA_behavior
import efr32bg12p.modules.PA.PA_behavior as PA_behavior
import efr32bg12p.modules.PRS.PRS_behavior as PRS_behavior
import efr32bg12p.modules.PTI.PTI_behavior as PTI_behavior
import efr32bg12p.modules.PYD1698.PYD1698_behavior as PYD1698_behavior
import efr32bg12p.modules.SERIAL.SERIAL_behavior as SERIAL_behavior
import efr32bg12p.modules.SPIDISPLAY.SPIDISPLAY_behavior as SPIDISPLAY_behavior
import efr32bg12p.modules.SPINCP.SPINCP_behavior as SPINCP_behavior
import efr32bg12p.modules.TIMER0.TIMER_behavior as TIMER_behavior
import efr32bg12p.modules.TIMER1.TIMER_behavior as TIMER_behavior
import efr32bg12p.modules.UARTNCP.UARTNCP_behavior as UARTNCP_behavior
import efr32bg12p.modules.USART0.USART_behavior as USART_behavior
import efr32bg12p.modules.USART1.USART_behavior as USART_behavior
import efr32bg12p.modules.USART2.USART_behavior as USART_behavior
import efr32bg12p.modules.USART3.USART_behavior as USART_behavior
import efr32bg12p.modules.VCOM.VCOM_behavior as VCOM_behavior
import efr32bg12p.modules.VUART.VUART_behavior as VUART_behavior
import efr32bg12p.modules.WDOG.WDOG_behavior as WDOG_behavior
def generate(context):
    ...
    for transition in modeTransitions():
        ...
        add_peripheral_defines(ExporterModel.module_list, transition, define_list, routes)
    ...
    try:
        ...
        refresh_studio_modules(framework, ExporterModel.module_list)
    except:
...
def add_prs_defines(prs_mod, properties, define_list, routes):
    ...
    for chan, options in prs_properties.items():
        if options["source"] and options["signal"]:
            define_list.append({"region": def_region, "label": "BSP_PRS_CH{}_SOURCE".format(chan), "def_value": "PRS_CH_CTRL_SOURCESEL_{}".format(options["source"])})
            define_list.append({"region": def_region, "label": "BSP_PRS_CH{}_SIGNAL".format(chan), "def_value": "PRS_CH_CTRL_SIGSEL_{}".format(options["signal"].replace("_", ""))})
            define_list.append({"region": def_region, "label": "BSP_PRS_CH{}_SOURCESIGNAL".format(chan),"def_value": "PRS_{}".format(options["signal"])})
        if options["pin"]:
        ...
        if options["custom_name"]:
            ...
            if options["source"] and options["signal"]:
                ...
                define_list.append({"region": def_region, "label": "BSP_PRS_{}_SOURCE".format(options["custom_name"]), "def_value": "PRS_CH_CTRL_SOURCESEL_{}".format(options["source"])})
                define_list.append({"region": def_region, "label": "BSP_PRS_{}_SIGNAL".format(options["custom_name"]),"def_value": "PRS_CH_CTRL_SIGSEL_{}".format(options["signal"].replace("_", ""))})
                define_list.append({"region": def_region, "label": "BSP_PRS_{}_SOURCESIGNAL".format(options["custom_name"]),"def_value": "PRS_{}".format(options["signal"])})
            if options["pin"]:
                if options["route"]["common_route"]:
