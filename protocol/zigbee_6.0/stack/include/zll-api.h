...
/**
 * @brief This call is fired when a target device is preparing a response
 * to a network start or network join router request, and provides the
 * opportunity to modify certain fields in the response message.
 *
 * @param cmdId is the touchlink command id.
 * @param transactionId is the touchlink transaction id.
 * @param responseStatus is the touchlink response status.
 * @return Returns true if no further processing is required.
 */
...
/**
 * @brief This call is fired when a target device is about to send
 * a unicast response to a network start or network join router request,
 * and provides the opportunity to modify the MAC header.
 *
 * @param targetEui64 is the destination long address.
 * @param buffer contains the MAC payload.
 * @param length is the length in bytes of the MAC payload.
 * @return Returns true if no further processing is required.
 */
...
/**
 * @brief This call will get the policy that enables or disables ZLL processing.
 */
EmberZllPolicy emberZllGetPolicy(void);

/**
 * @brief This call will set the policy to enable or disable ZLL processing.
 */
