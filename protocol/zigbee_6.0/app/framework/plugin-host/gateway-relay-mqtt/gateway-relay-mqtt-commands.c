...
static void scanEndpoints(
  ...
  EmberAfPluginCommandRelayDeviceEndpoint *);
static bool commandCompare(const uint8_t * targetString,
                           ...
                           const uint8_t * sourceString)
{
  ...
  for (i = 0; i < strnlen(targetString, MAX_COMMAND_LEN); i++) {
...
void emAfPluginGatewayRelayMqttCommandsProccessing(uint8_t * inputString)
{
  ...
  for (index = 0; index < NUMBER_OF_COMMANDS; index++) {
    if (commandCompare(commandArray[index].commandString, inputString)) {
      argStartPosition =
