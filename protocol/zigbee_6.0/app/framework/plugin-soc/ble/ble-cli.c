...
void emberAfPluginBleGetAddressCommand(void)
{
  ...
  emberAfCorePrintln("BLE address: [%x %x %x %x %x %x]",
                     rsp->address.addr[0], rsp->address.addr[1],
                     rsp->address.addr[2], rsp->address.addr[3],
                     rsp->address.addr[4], rsp->address.addr[5]);
}
...
void emberAfPluginBleEndpointCloseCommand(void)
{
  uint8_t connectionHandle = emberUnsignedCommandArgument(0);
