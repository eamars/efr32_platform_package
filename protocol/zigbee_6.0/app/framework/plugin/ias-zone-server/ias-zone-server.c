...
#define QUEUEING_BACKOFF_DELAY_MS_ATTEMPT_1  (3 * MILLISECOND_TICKS_PER_SECOND)
#define QUEUEING_BACKOFF_DELAY_MS_ATTEMPT_2  (5 * MILLISECOND_TICKS_PER_SECOND)
#define QUEUEING_BACKOFF_DELAY_MS_ATTEMPT_N  (10 * MILLISECOND_TICKS_PER_SECOND)
#define QUEUEING_RETRY_MINIMUM_OTA_TIME_S    3
#define QUEUEING_RETRY_MINIMUM_OTA_TIME_QS \
  (QUEUEING_RETRY_MINIMUM_OTA_TIME_S * 4)
#define QUEUEING_RETRY_MINIMUM_OTA_TIME_MS \
  (QUEUEING_RETRY_MINIMUM_OTA_TIME_S * MILLISECOND_TICKS_PER_SECOND)
...
static int16_t popFromBuffer(const IasZoneStatusQueue *ring,
  ...
  uint16_t timeSinceStatusOccurredQs)
{
  ...
  if (messageQueue.entriesInQueue == 0) {
    ...
    // Even though this event won't cause a new command to be transmitted, it
    // should still check to see if it should restart the parent search pattern
    copyToBuffer(&messageQueue, &newBufferEntry);
...
void emberAfPluginIasZoneServerManageQueueEventHandler(void)
{
  ...
  // re-send a message if it's been at least 3 seconds since it was sent.
  bufferStart = &(messageQueue.buffer[messageQueue.startIdx]);
  elapsedTimeQs = computeElapsedTimeQs(bufferStart);

  if (elapsedTimeQs < QUEUEING_RETRY_MINIMUM_OTA_TIME_QS) {
    airTimeRemainingMs = QUEUEING_RETRY_MINIMUM_OTA_TIME_MS
    ...
    emberAfIasZoneClusterPrintln(
      "Attempting to resend a queued zone status update with time of %d", elapsedTimeQs);
  ...
  EmberStatus status)
{
  ...
  // The event handler will perform the retransmit, and the original send
  // request will handle populating the buffer.
  if (status == EMBER_DELIVERY_FAILED) {
    switch (retryAttempt) {
      case 0:
        delayMs = QUEUEING_BACKOFF_DELAY_MS_ATTEMPT_1;
        retryAttempt++;
        break;
      case 1:
        delayMs = QUEUEING_BACKOFF_DELAY_MS_ATTEMPT_2;
        retryAttempt++;
        break;
      default:
        delayMs = QUEUEING_BACKOFF_DELAY_MS_ATTEMPT_N;
    }
    emberAfIasZoneClusterPrint(
      "Status command update failed to send.  retrying in %d ms",
      delayMs);
    emberEventControlSetDelayMS(
      emberAfPluginIasZoneServerManageQueueEventControl,
      delayMs);
  } else if (status == EMBER_SUCCESS) {
    // If a command message was sent, remove it from the queue and move on to
    // the next queued message until the queue is empty.
    emberAfIasZoneClusterPrintln(
