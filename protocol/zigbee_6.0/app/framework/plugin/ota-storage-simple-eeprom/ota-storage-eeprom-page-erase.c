...
void emAfOtaStorageEepromInit(void)
{
  ...
  assert(info != NULL);
  // Since this is just a series of asserts, it's not worth turning this into
  // a null check for CSTAT appeasement. Let's just ignore the warning here.
  // info cannot be null due to the above assert.
  //cstat !PTR-null-cmp-bef
...
EmberAfOtaStorageStatus emberAfOtaStorageDriverPrepareToResumeDownloadCallback(void)
{
  ...
  // Resuming OTA downloads does not currently work when using slots
  // This will be fixed in a future release
  return EMBER_AF_OTA_STORAGE_ERROR;
#else // OTA with no slots
  ...
  return (startEraseOperation(pageOffsetStart,
          ...
          ? EMBER_AF_OTA_STORAGE_OPERATION_IN_PROGRESS
          : EMBER_AF_OTA_STORAGE_ERROR);
#endif // (SLOT_STRATEGY != DO_NOT_USE_SLOTS)
}
