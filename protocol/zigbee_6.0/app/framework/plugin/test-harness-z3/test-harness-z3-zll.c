...
// Interpan packet constants
#define EMBER_ZLL_INTERPAN_OVERHEAD 14  // NWK/APS/ZLL headers
#define EM_ZLL_NETWORK_START_RESPONSE_LENGTH                (EMBER_ZLL_INTERPAN_OVERHEAD + 13)

// Network Join Router/End Device Response data
#define EM_ZLL_NETWORK_JOIN_RESPONSE_LENGTH                 (EMBER_ZLL_INTERPAN_OVERHEAD + 1)
...
// copied from interpan.c for writing MAC header
...
void emAfPluginTestHarnessZ3TouchlinkDeviceInformationRequestCommand(void)
{
  ...
  if (zllNetworkIsInit()) {
    ...
    status = emberAfSendCommandInterPan(0xFFFF,                // destination pan id
                                        ...
                                        emAfPluginTestHarnessZ3ProfileId);
  }
...
void emAfPluginTestHarnessZ3TouchlinkScanResponseConfigCommand(void)
{
  ...
  globalScanResponseData.profileId = emAfPluginTestHarnessZ3ProfileId;
...
#if defined EMBER_APPLICATION_HAS_ZLL_NETWORK_RESPONSE_TEST_HANDLER
boolean emberZllSendNetworkResponseTestHandler(uint8_t *cmdId,
                                               uint32_t *transactionId,
                                               EmberStatus *responseStatus)
{
  if (responseBehaviorMask) {
    ...
    // Apply negative behaviour options to the response message -
    // currently, they are set one at a time, but we should be
    // supporting them as independent options
    if (responseBehaviorMask & NEGATIVE_BEHAVIOR_BAD_COMMAND_ID) {
      *cmdId = ZCL_NETWORK_JOIN_END_DEVICE_RESPONSE_COMMAND_ID;
      responseBehaviorMask &= ~NEGATIVE_BEHAVIOR_BAD_COMMAND_ID;
    }
    if (responseBehaviorMask & NEGATIVE_BEHAVIOR_BAD_STATUS) {
      *responseStatus = EMBER_ERR_FATAL;
      responseBehaviorMask &= ~NEGATIVE_BEHAVIOR_BAD_STATUS;
    }
...
#if defined EMBER_APPLICATION_HAS_ZLL_SEND_UNICAST_TEST_HANDLER
boolean emberZllSendUnicastTestHandler(uint8_t *targetEui64,
                                       uint8_t* buffer,
                                       uint8_t length)
{
  ...
  if (responseBehaviorMask) {
    if (length > EM_ZLL_NETWORK_START_RESPONSE_LENGTH) {
      emberAfCorePrintln("%p: %p: 0x%X",
                         TEST_HARNESS_Z3_PRINT_NAME,
                         "emberZllSendUnicastTestHandler error: ",
                         EMBER_MESSAGE_TOO_LONG);
      goto done;
    }

    uint16_t macFrameControl = LONG_DEST_FRAME_CONTROL | MAC_ACK_REQUIRED;
    uint16_t destPanId = BROADCAST_PAN_ID;
    uint16_t sourcePanId = emberGetPanId();
    EmberEUI64 sourceEui64;
    MEMMOVE(sourceEui64, emberGetEui64(), EUI64_SIZE);
    if (responseBehaviorMask & NEGATIVE_BEHAVIOR_BAD_SOURCE_ADDRESS) {
      responseBehaviorMask &= ~NEGATIVE_BEHAVIOR_BAD_SOURCE_ADDRESS;
      MEMMOVE(sourceEui64, badEui64, EUI64_SIZE);
    }

    // Buffer for the complete 802.15.4 message (not incl MAC CRC)
    // NOTE: this is for the largest response message we expect here
    uint8_t macMessage[MAX_INTER_PAN_MAC_SIZE + EM_ZLL_NETWORK_START_RESPONSE_LENGTH];

    // The message still needs a MAC header
    uint8_t *finger = macMessage;
    finger = pushInt16u(finger, macFrameControl);
    finger++;   // stack will write sequence number
    finger = pushInt16u(finger, destPanId);
    finger = pushEui64(finger, targetEui64);
    finger = pushInt16u(finger, sourcePanId);
    finger = pushEui64(finger, sourceEui64);

    // Copy the MAC payload
    MEMMOVE(finger, buffer, length);

    // Everything else should be set up at this stage
    emAfPluginInterpanSendRawMessage(MAX_INTER_PAN_MAC_SIZE + length, macMessage);
