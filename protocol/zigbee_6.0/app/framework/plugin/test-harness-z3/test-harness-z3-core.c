...
uint16_t emAfPluginTestHarnessZ3ProfileId = EMBER_ZLL_PROFILE_ID;
...
void emberAfPluginTestHarnessZ3OpenNetworkEventHandler(void)
{
  ...
  if (emberAfGetNodeId() == EMBER_TRUST_CENTER_NODE_ID) {
    ...
    // allows key requests.
#ifdef EZSP_HOST
    ezspAddTransientLinkKey(wildcardEui64, &centralizedKey);
...
{
  emAfPluginTestHarnessZ3ProfileId = (emberStringCommandArgument(-1, NULL)[0] == 'e')
