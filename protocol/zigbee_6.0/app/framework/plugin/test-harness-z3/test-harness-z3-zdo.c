                                                             ...
                                                             EmberApsFrame *apsFrame)
{
  ...
  // Command payload ength to be checked before calling
  // emberGetLinkedBuffersByte - it will assert for bad boundary.
  uint8_t commandLength = emberMessageBufferLength(requestBuffer) - commandIndex;
  if (clusterId == IEEE_ADDRESS_REQUEST) {
    if (commandLength <  TH_IEEE_ADD_REQ_PAYLOAD_LENGTH ) {
      return EMBER_INVALID_CALL;
    }
    ...
    if (commandLength < TH_SIMPLE_DESC_REQ_PAYLOAD_LENGTH ) {
      // short length - return.
      return EMBER_INVALID_CALL;
    }
    uint8_t endpoint
