...
#if defined EMBER_APPLICATION_HAS_OUTGOING_COMMAND_HANDLER
// Allow the framework to modify the beacon, for negative conformance test cases.
// The function is called from our implementation of emberOutgoingCommandHandler.
void emAfPluginTestHarnessZ3ModifyBeaconPayload(uint8_t *beaconPayload, uint8_t *payloadLength)
{
  if (beaconConfigNegativeBehaviorMask) {
    emberAfCorePrintln("Modifying beacon, mask = %4X", beaconConfigNegativeBehaviorMask);
