                                                  ...
                                                  EmberAfGbzZclCommand * gbzZclCommand)
{
  ...
  if (state->componentsParsed < state->componentsSize) {
    ...
    gbzZclCommand->clusterId = (state->command[state->parseIndex] << 8) | state->command[state->parseIndex + 1];
    state->parseIndex += 2;

    gbzCmdLength = (state->command[state->parseIndex] << 8) | state->command[state->parseIndex + 1];
    state->parseIndex += 2;
    ...
    if (encryption) {
      ...
      data.encryptedPayloadLength = emberAfGetInt16u(state->command, state->parseIndex, state->length);
  ...
  if (!resp->clusterSpecific) {
    ...
    switch (creator->messageCode) {
      ...
      case GCS15d_MESSAGE_CODE:
        ...
        break;
      case GCS15e_MESSAGE_CODE:
        if ((*clusterId == ZCL_PREPAYMENT_CLUSTER_ID)
            && (*commandId == ZCL_PUBLISH_TOP_UP_LOG_COMMAND_ID)) {
          return true;
        }
        break;
