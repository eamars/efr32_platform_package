...

{
  ...
  if (phyIndexRequiredCounter
      && (phyIndex < EMBER_MAX_NUM_PHYS)
      && (type < EMBER_MAX_MAC_ONLY_COUNTERS)) {
    ...
    if (EMBER_COUNTER_MAC_TX_UNICAST_SUCCESS == type
      ...
      if ((emberMacCounters[phyIndex][EMBER_COUNTER_MAC_TX_UNICAST_RETRY] + data)

          < emberCountersThresholds[EMBER_COUNTER_MAC_TX_UNICAST_RETRY]) {
        emberMacCounters[phyIndex][EMBER_COUNTER_MAC_TX_UNICAST_RETRY] += data;
      } else {
        emberMacCounters[phyIndex][EMBER_COUNTER_MAC_TX_UNICAST_RETRY] =
