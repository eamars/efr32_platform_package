...
//Linklayer task
#define LINKLAYER_STACK_SIZE (700 / sizeof(CPU_STK))
...
void BluetoothLLCallback(RTOS_ERR *os_err)
{
...
void BluetoothUpdate(RTOS_ERR *os_err)
{
...
void BluetoothTask(void *p)
{
  ...
  while (DEF_TRUE) {
    ...
    if (flags & BLUETOOTH_EVENT_FLAG_CMD_WAITING) {
      gecko_handle_command(command_header, (void*)command_data);
...
{
  rtos_gecko_handle_command(header, payload);
}
