...
static ClientTable clientTable[EMBER_MAX_END_DEVICE_CHILDREN] = { 0 };
                                        ...
                                        uint8_t *messageContents)
{
  ...
  } else if (dcState >= EMBER_DUTY_CYCLE_LBT_CRITICAL_THRESHOLD_REACHED) {
    // In "Critical" Duty Cycle state, do not allow any suspension to expire
                                  ...
                                  EmberPerDeviceDutyCycle *arrayOfDutyCycles)
{
  ...
  if (emberAfPluginSubGhzDutyCycleCallback(channelPage,
    ...
    EmberNodeId chosenNodeId = EMBER_NULL_NODE_ID;
    uint8_t i;
