...
EmberStatus emberAfZllSetInitialSecurityState(void)
{
  ...
  // If we are a coordinator, we don't want to change the security settings of our centralized network
  if (emAfCurrentZigbeeProNetwork->nodeType == EMBER_COORDINATOR) {
    emberZllSetSecurityStateWithoutKey(&securityState);
...
static bool amFactoryNew(void)
{
  EmberTokTypeStackZllData token;
  emberZllGetTokenStackZllData(&token);
  return isFactoryNew(token.bitmask) && emAfCurrentZigbeeProNetwork->nodeType != EMBER_COORDINATOR;
}
