...
static uint16_t gMinimumBlockPeriodSec = 0;
...
uint8_t emAfOtaServerImageBlockRequestCallback(EmberAfImageBlockRequestCallbackStruct* data)
{
  if (SERVER_AND_CLIENT_SUPPORT_MIN_BLOCK_REQUEST
    ...
    if (data->minBlockRequestPeriod != gMinimumBlockPeriodSec) {
      data->minBlockRequestPeriod = gMinimumBlockPeriodSec;
...
{
  gMinimumBlockPeriodSec = minBlockRequestPeriodSec;
}
