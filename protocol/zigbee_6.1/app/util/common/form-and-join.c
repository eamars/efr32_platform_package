...
#if defined(EMBER_AF_PLUGIN_SUB_GHZ_SERVER)
...
bool emberEnableDualChannelScan = true;
...
{
#if defined(EMBER_AF_PLUGIN_SUB_GHZ_CLIENT) || defined(EMBER_AF_PLUGIN_SUB_GHZ_SERVER)
  ...
  switch (emberAfGetFormAndJoinSearchMode()) {
    case FIND_AND_JOIN_MODE_ALLOW_BOTH:
      if (currentPage == 0) {
        ...
        if (phyInterfaceCount > 1 && phyInterfaceCount != 0xFF) {
          ...
          // Continue on the first sub-GHz page (minus 1 because we are increment
    ...
    case FIND_AND_JOIN_MODE_ALLOW_SUB_GHZ:
      ...
      if (currentPage < EMBER_MAX_SUGBHZ_PAGE_NUMBER) {
        ...
        SCAN_DEBUG_MSG("%d\n", currentPage);
        if (startScanForUnusedPanId((((uint32_t)currentPage) << EMBER_MAX_CHANNELS_PER_PAGE) | emberAfGetFormAndJoinChannelMask(currentPage),
...
static void startPanIdScan(void)
{
  ...
  // startScan().
  const uint32_t pageMask = (channelCache & 0xE0)
                            ? (EMBER_MIN_SUGBHZ_PAGE_NUMBER + ((channelCache >> 5) & 0x03)) << EMBER_MAX_CHANNELS_PER_PAGE
                            : 0;
  uint8_t i = 0;

  while (i < NUM_PAN_ID_CANDIDATES) {
    uint16_t panId = halCommonGetRandom() & 0xFFFF;
    if (panId != 0xFFFF) {
  ...
  startScan(EMBER_ACTIVE_SCAN, pageMask | BIT32(channelCache & 0x1F), ACTIVE_SCAN_DURATION);
}
...
{
  EmberStatus status;
  const uint8_t page = EMBER_MIN_SUGBHZ_PAGE_NUMBER + ((channelCache >> 5) & 0x03);
  const uint8_t channel = channelCache & 0x1F;
...
EmberStatus emberScanForNextJoinableNetwork(void)
{
  ...
  // one 2.4GHz and four sub-GHz, and some may pages have a different number of
  ...
  while (channelCache < EMBER_MAX_CHANNELS_PER_PAGE) {
    ...
    if ((bitMask & channelMaskCache) != 0U) {
      ...
      return startScan(EMBER_ACTIVE_SCAN, pageMask | bitMask, ACTIVE_SCAN_DURATION);
    }
  }
  ...
  switch (formAndJoinScanType) {
    ...
    case FORM_AND_JOIN_JOINABLE_SCAN:
      ...
      if (emberEnableDualChannelScan
          ...
          && networkCount) {
        uint8_t dualChannel = (channelCache + (channelCache < 15 ? 12 : -12));
