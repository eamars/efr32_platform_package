 ...
 */
EmberStatus emberRf4ceSetPairingTableEntry(uint8_t pairingIndex,
 ...
 */
EmberStatus emberRf4ceGetPairingTableEntry(uint8_t pairingIndex,
                                           EmberRf4cePairingTableEntry *entry);
 ...
 * This API can be called before and after the node has been started.
 */
...
/** @brief Retrieves the device base channel as specified in the RF4CE specs.
 *
 * @return   The base channel of the device.
 */
 ...
 *                        ::emberRf4ceMessageSentHandler() call.
 *
 ...
 * - :: EMBER_MESSAGE_TOO_LONG - If the passed message is too long. Notice that
 *   the maximum payload allowed for a certain combination of pairing index and
 *   TX options can be retrieved using the ::emberRf4ceGetMaxPayload() API.
 ...
 *  layers.
 */
EmberStatus emberRf4ceSend(uint8_t pairingIndex,
                           uint8_t profileId,
                           uint16_t vendorId,
...
/** @brief An API that returns the maximum payload of an RF4CE network DATA
 *  frame according to the passed pairing index and TX options.
 *
 *  @param pairingIndex   The index of the pairing table entry a packet shall be
 *                        sent to. This parameter is meaningful only if the
 *                        EMBER_RF4CE_TX_OPTIONS_BROADCAST_BIT bit in the passed
 *                        TX options is not set.
 *
 *  @param txOptions      The TX options bitmask.
 *
 *  @return   The maximum allowed payload in bytes according to the passed
 *  pairing index and TX options. Notice that if the
 *  EMBER_RF4CE_TX_OPTIONS_BROADCAST_BIT bit is not set in the TX options
 *  bitmask and the passed pairing index is not valid, 0 is returned.
 */
uint8_t emberRf4ceGetMaxPayload(uint8_t pairingIndex,
                                EmberRf4ceTxOption txOptions);
 ...
 * node is a non-sleepy ZigBee PRO node or a ZigBee RF4CE node). The application
 * can set the application information using the
 * ::emberRf4ceSetApplicationInfo() API.
 */
 ...
 * node was not cold started with the ::emberRf4ceStart() or warm started with
 ...
 * the application wants to respond to the discovery request. The application
 * can set the application information of the discovery response within this
 * callback using the  ::emberRf4ceSetApplicationInfo() API.
 */
 ...
 * discovery or pairing process. The application can set the application
 * information used during the auto discovery process using the
 * ::emberRf4ceSetApplicationInfo() API.
 */
 ...
 *
 * @param keyExchangeTransferCount  The number of transfers the target should
 ...
 */
EmberStatus emberRf4cePair(uint8_t channel,
                           EmberPanId panId,
                           EmberEUI64 ieeeAddr,
 ...
 *                      status parameter is EMBER_SUCCESS or
 *                      EMBER_DUPLICATE_ENTRY.
 *
 ...
 *                      EMBER_SUCCESS or EMBER_DUPLICATE_ENTRY.
 *
 * @param appInfo       A pointer to an ::EmberRf4ceApplicationInfo struct
 *                      containing the application information of the peer node.
 *                      This parameter is non-NULL only if the status parameter
 *                      is EMBER_SUCCESS or EMBER_DUPLICATE_ENTRY.
 */
 ...
 *                                  request command.
 *
 * @return   true if the application accepts the pair, false otherwise. The
 * application can set the application information of the pair response
 * within this callback using the  ::emberRf4ceSetApplicationInfo() API.
 */
 ...
 *                      are 0x000000 and any value in the range
 ...
 */
EmberStatus emberRf4ceSetPowerSavingParameters(uint32_t dutyCycle,
 ...
 */
EmberStatus emberRf4ceSetFrequencyAgilityParameters(uint8_t rssiWindowSize,
