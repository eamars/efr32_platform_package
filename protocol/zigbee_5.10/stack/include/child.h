...
/*
 * return the incoming frame counter of current parent node */
...
uint32_t emberGetParentIncomingNWKFrameCounter(void);
EmberStatus emberSetParentAndIncomingNWKFrameCounter(uint32_t value);
EmberNodeId emberGetParentNodeId(void);
...
enum {
  ...
  /** The node is busy performing some ZigBee RF4CE network operation. */
  EMBER_RF4CE_NETWORK_BUSY = 0x0080,
  /** The node is in RF4CE duty-cycling mode. */
  EMBER_RF4CE_DUTY_CYCLING_ENABLED = 0x0100,
