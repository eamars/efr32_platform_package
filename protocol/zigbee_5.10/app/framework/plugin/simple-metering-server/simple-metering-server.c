...
static void createReportingAttributes(void);
...
// This function will create reporting entries for instantaneous demand power
// and cumulative energy consumption
static void createReportingAttributes(void)
{
#ifdef EMBER_AF_PLUGIN_REPORTING
  uint8_t i, endpoint;
  EmberAfPluginReportingEntry cumulativeEnergyReportingEntry = {
    ...
    .data.reported = {
      10,        // minimum interval (second)
      1000,      // maximum interval (second)
      10         // report when this amount changes
    }
  };
#ifdef ZCL_USING_SIMPLE_METERING_CLUSTER_INSTANTANEOUS_POWER_ATTRIBUTE
  EmberAfPluginReportingEntry instantaneousPowerReportingEntry = {
    EMBER_ZCL_REPORTING_DIRECTION_REPORTED,
    0, // this will be populated below
    ZCL_SIMPLE_METERING_CLUSTER_ID,
  ...
  for (i = 0; i < emberAfEndpointCount(); i++) {
    ...
    if (emberAfContainsServer(endpoint, ZCL_ON_OFF_CLUSTER_ID)) {
      cumulativeEnergyReportingEntry.endpoint = endpoint;
      emberAfPluginReportingConfigureReportedAttribute(
        &cumulativeEnergyReportingEntry);
#ifdef ZCL_USING_SIMPLE_METERING_CLUSTER_INSTANTANEOUS_POWER_ATTRIBUTE
      instantaneousPowerReportingEntry.endpoint = endpoint;
      emberAfPluginReportingConfigureReportedAttribute(
        &instantaneousPowerReportingEntry);
#endif
    }
  }
#endif
