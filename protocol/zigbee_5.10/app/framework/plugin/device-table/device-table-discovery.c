...
#define ENDPOINT_DESCRIPTOR_PROFILE_ID_LOCATION \
  (ENDPOINT_DESCRIPTOR_ENDPOINT_LOCATION        \
   + 1 /* endpoint */                           \
  )

#define ENDPOINT_DESCRIPTOR_DEVICE_ID_LOCATION \
  (ENDPOINT_DESCRIPTOR_PROFILE_ID_LOCATION + 2)

#define ENDPOINT_DESCRIPTOR_INPUT_CLUSTER_LIST_COUNT_INDEX \
  (ENDPOINT_DESCRIPTOR_DEVICE_ID_LOCATION                  \
   + 2  /* device ID length */                             \
   + 1  /* version (4-bits), reserved (4-bits) */          \
  )

#define ENDPOINT_DESCRIPTOR_INPUT_CLUSTER_LIST_INDEX \
  (ENDPOINT_DESCRIPTOR_INPUT_CLUSTER_LIST_COUNT_INDEX + 1)
                                                     ...
                                                     uint16_t length)
{
  ...
  uint16_t endpointIndex = emAfDeviceTableFindIndexNodeIdEndpoint(nodeId, endpoint);
  EmberAfPluginDeviceTableEntry *pEntry;
  uint8_t clusterIndex = 0;
  uint8_t i, currentClusterType, ClusterCount;
  uint8_t msgArrayIndex = ENDPOINT_DESCRIPTOR_INPUT_CLUSTER_LIST_INDEX;

  endpoint = message[ENDPOINT_DESCRIPTOR_ENDPOINT_LOCATION];
  ...
  ClusterCount = message[ENDPOINT_DESCRIPTOR_INPUT_CLUSTER_LIST_COUNT_INDEX];

  for (currentClusterType = CLUSTER_IN;
       currentClusterType < NUMBER_OF_CLUSTER_IN_OUT;
       currentClusterType++) {
    for (i = 0; i < ClusterCount; i++) {
      pEntry->clusterIds[clusterIndex] = message[msgArrayIndex]
                                         + (message[msgArrayIndex + 1] << 8);
      emberAfCorePrintln("%p, Cluster: 0x%2x",
                         currentClusterType ? "Out" : "In",
    ...
    if (currentClusterType == CLUSTER_IN) {
      ...
      emberAfCorePrintln("out Cluster starts at %d",
                         pEntry->clusterOutStartPosition);
