...
{
  uint8_t physEnv = (emAfPluginMeterMirrorGetMirrorsAllocated()
...
{
  uint8_t endpoint = index + EMBER_AF_PLUGIN_METER_MIRROR_ENDPOINT_START;
...
uint16_t emberAfPluginMeterMirrorRequestMirror(EmberEUI64 requestingDeviceIeeeAddress)
{
  ...
  if (!physEnv && index == INVALID_INDEX) {
                                     ...
                                     uint16_t bufLen)
{
  ...
  while (bufIndex + ATTRIBUTE_OVERHEAD < bufLen) {
...

    {
      ...
      if (attributeId == ZCL_SIMPLE_METERING_CLUSTER_REPORTING_STATUS_ATTRIBUTE_ID) {
