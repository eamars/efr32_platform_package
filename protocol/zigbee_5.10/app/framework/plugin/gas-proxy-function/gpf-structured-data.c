                           ...
                           GpfSampleLog *sampleLog)
{
  ...
  for (i = 0, entryIndex = getFirstIndex(sampleLog);
    ...
    //GBCS IRP 338: Inclusion of ZCL Default Response commands in Responses:
    //The ZSE Disable Default Response bit should not be set in any message sent to a device.
...
void emberAfPluginMeterMirrorReportingCompleteCallback(uint8_t endpoint)
{
  ...
  uint32_t currentDayAlternativeConsumption;
  uint32_t currentDayCostConsumption;
  uint32_t now = emberAfGetCurrentTime();
