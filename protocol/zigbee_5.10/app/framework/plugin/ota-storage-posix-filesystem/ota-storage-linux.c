...
static const uint16_t otaFileVersionNumber = 0x0100;
...
static uint8_t* writeHeaderDataToBuffer(EmberAfOtaHeaderFieldDefinition* definition,
                                        uint8_t* bufferPtr);
                                                  ...
                                                  const char* filename)
{
  ...
  bufferPtr += 4;
  bufferPtr[0] = (uint8_t)otaFileVersionNumber;
  bufferPtr[1] = (uint8_t)(otaFileVersionNumber >> 8);
  bufferPtr += 2;
                                                        ...
                                                        int32_t actualBufferDataRemaining)
{
  if (definition->maskForOptionalField != ALWAYS_PRESENT_MASK) {
    uint16_t fieldControl = *(uint16_t*)(otaHeaderFieldDefinitions[FIELD_CONTROL_INDEX].location);
  ...
  if (definition->type == INTEGER_FIELD) {
    ...
    if (1 == definition->length) {
      uint8_t *value = (uint8_t*)(definition->location);
      *value = bufferPtr[0];
    } else if (2 == definition->length) {
      uint16_t *value = (uint16_t*)(definition->location);
      *value = (bufferPtr[0]
                + (bufferPtr[1] << 8));
    } else if (4 == definition->length) {
      uint32_t *value = (uint32_t*)(definition->location);
      *value = (bufferPtr[0]
                + (bufferPtr[1] << 8)
                + (bufferPtr[2] << 16)
                + (bufferPtr[3] << 24));
...
uint32_t emAfOtaStorageGetSlot()
{
  return INVALID_SLOT;
}
