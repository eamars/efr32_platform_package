...
#ifdef EMBER_TEST
  //we think the code should only get here in case of simulation
  uint8_t reset;
  reset = halGetResetInfo();
  if (reset == RESET_SOFTWARE_EM4) {
    // This could only be called if idle-sleep is in use, and em4 is ok
    emberAfCameBackFromEM4Callback();
  }
#else
  uint16_t extReset;
  extReset = halGetExtendedResetInfo();
  emberAfCorePrintln("extReset :%d , RESET_SOFTWARE_EM4 : %d, RESET_EXTERNAL_EM4PIN= %d", extReset, RESET_SOFTWARE_EM4, RESET_EXTERNAL_EM4PIN);
  if (extReset == RESET_SOFTWARE_EM4
      || extReset == RESET_EXTERNAL_EM4PIN ) {
    emberAfCameBackFromEM4Callback();
  }
#endif
