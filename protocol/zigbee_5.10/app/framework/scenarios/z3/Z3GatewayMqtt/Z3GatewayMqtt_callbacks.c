...
static cJSON* buildNodeJson(uint16_t nodeIndex)
{
  ...
  deviceEndpoint = buildDeviceEndpointWithClusterInfo(deviceTable[nodeIndex].eui64,
...
static void publishMqttRelays(void)
{
  ...
  for (i = 0; i < EMBER_AF_PLUGIN_COMMAND_RELAY_RELAY_TABLE_SIZE; i++) {
    ...
    deviceEndpointJson = buildDeviceEndpointWithClusterId(relayTable[i].inDeviceEndpoint.eui64,
    ...
    deviceEndpointJson = buildDeviceEndpointWithClusterId(relayTable[i].outDeviceEndpoint.eui64,
