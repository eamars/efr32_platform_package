...
#if defined(EMBER_PLUGIN_EM4)
#if !defined((EMBER_SUPPORTED_NETWORKS != 1))
    ...
    if (emberOkToNap()) {
      ...
      if (EMBER_AF_PLUGIN_IDLE_SLEEP_MINIMUM_SLEEP_DURATION_MS <= durationMs
        ...
        if (!emberAfPluginIdleSleepRtosCallback(&durationMs, true)) {
          halPowerDown();
          // This is normal sleeping mode before EM4
#ifdef EMBER_ENABLE_EM4
          // Additional conditions must be implemented in emberAfOKToGoToEM4Callback()? e.g.
          // return emberOkToHibernate() or return (emberCurrentStackTasks() & EMBER_ASSOCIATING) or etc.?
          if (!emberAfOKToGoToEM4Callback() || !emberOkToGoToLowPowerMode()) {
            halSleepForMilliseconds(&durationMs);
          } else {
            emberPluginEM4AboutToGoInEM4Callback(durationMs); //this would need to call something in the user/app code
...
{
  // this must call that emberPluginEM4ComingBackFromEM4Callback
  // this is the part that chains the em4 plugin to the idle-sleep
#ifdef EMBER_ENABLE_EM4
  emberPluginEM4ComingBackFromEM4Callback();
#endif
}

// EMBER_NETWORK_INIT_PARENT_INFO_IN_TOKEN is not an optional macro, so it could not be checked at the begining.
// In general, we would need to check for EMBER_NETWORK_INIT_PARENT_INFO_IN_TOKEN,
// if we don't initiate it by default
void emberAfPluginIdleSleepInitCallback(void)
{
  ...
  emberNetworkInitExtended(&networkInitStruct);
#endif
}
