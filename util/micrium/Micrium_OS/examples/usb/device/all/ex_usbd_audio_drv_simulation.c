...
#include  <rtos/usb/include/device/usbd_audio_processing.h>
...
*/
                                                                ...
                                                                /* Audio class example tasks settings.                  */
#define  EX_USBD_AUDIO_DRV_SIMULATION_MIC_TASK_PRIO                5u
#define  EX_USBD_AUDIO_DRV_SIMULATION_LOOP_TASK_PRIO               5u
#define  EX_USBD_AUDIO_DRV_SIMULATION_MIC_TASK_STK_SIZE         1024u
#define  EX_USBD_AUDIO_DRV_SIMULATION_LOOP_TASK_STK_SIZE        1024u
...
#define  USBD_AUDIO_DRV_SIMULATION_CIRCULAR_AUDIO_BUF_SIZE        20u
...
#define  USBD_AUDIO_DRV_SIMULATION_LOOP_MAX_NB_XFERS              10u
...
CPU_INT08U  Ex_USBD_AudioMic_IT_ID             = USBD_AUDIO_INVALID_ID;
CPU_INT08U  Ex_USBD_AudioMic_OT_USB_IN_ID      = USBD_AUDIO_INVALID_ID;
CPU_INT08U  Ex_USBD_AudioMic_FU_ID             = USBD_AUDIO_INVALID_ID;
CPU_INT08U  Ex_USBD_AudioSpeaker_IT_USB_OUT_ID = USBD_AUDIO_INVALID_ID;
CPU_INT08U  Ex_USBD_AudioSpeaker_OT_ID         = USBD_AUDIO_INVALID_ID;
CPU_INT08U  Ex_USBD_AudioSpeaker_FU_ID         = USBD_AUDIO_INVALID_ID;
...
static         USBD_AUDIO_DRV_SIMULATION_DATA               Ex_USBD_Audio_DrvData;
static         USBD_AUDIO_DRV_SIMULATION_CIRCULAR_BUF       Ex_USBD_Audio_DrvCircularBufRecord;

static         USBD_AUDIO_DRV_SIMULATION_CIRCULAR_BUF       Ex_USBD_Audio_DrvCircularBufPlayback;
static         USBD_AUDIO_DRV_SIMULATION_LOOP_CIRCULAR_BUF  Ex_USBD_Audio_DrvCircularBufLoopback;

extern  const  CPU_INT16U                                   Ex_USBD_Audio_DrvSimulationDataNbSamples44_1;
extern  const  CPU_INT16U                                   Ex_USBD_Audio_DrvSimulationDataNbSamples48;
extern  const  CPU_INT16U                                   Ex_USBD_Audio_DrvSimulationDataTbl44_1[];
extern  const  CPU_INT16U                                   Ex_USBD_Audio_DrvSimulationDataTbl48[];
...
static  OS_TCB  USBD_Audio_DrvSimulationMicTaskTCB;
static  OS_TCB  USBD_Audio_DrvSimulationLoopTaskTCB;
...
static  void         USBD_Audio_MicDrvInit                     (USBD_AUDIO_DRV        *p_audio_drv,
                                                                RTOS_ERR              *p_err);

static  void         USBD_Audio_LoopDrvInit                    (USBD_AUDIO_DRV        *p_audio_drv,
                                                                RTOS_ERR              *p_err);
...
static  CPU_BOOLEAN  USBD_Audio_DrvAS_SamplingFreqSpeakerManage(USBD_AUDIO_DRV        *p_audio_drv,
...
#error  "USBD_AUDIO_CFG_PLAYBACK_EN or USBD_AUDIO_CFG_RECORD_EN must be DEF_ENABLED for audio demo."
#endif
...
const  USBD_AUDIO_DRV_COMMON_API  Ex_USBD_Audio_MicDrvCommonAPI_Simulation = {
    USBD_Audio_MicDrvInit
};

const  USBD_AUDIO_DRV_COMMON_API  Ex_USBD_Audio_LoopDrvCommonAPI_Simulation = {
    USBD_Audio_LoopDrvInit
};

const  USBD_AUDIO_DRV_AC_FU_API  Ex_USBD_Audio_DrvFU_API_Simulation = {
...
const  USBD_AUDIO_DRV_AS_API  Ex_USBD_Audio_MicDrvAS_API_Simulation = {
...
const  USBD_AUDIO_DRV_AS_API  Ex_USBD_Audio_SpeakerDrvAS_API_Simulation = {
    USBD_Audio_DrvAS_SamplingFreqSpeakerManage,
    DEF_NULL,
    USBD_Audio_DrvStreamStart,
    USBD_Audio_DrvStreamStop,
    USBD_Audio_DrvStreamRecordRx,
...
*
*                               RTOS_ERR_NONE       Audio device     successfully initialized.
*                               RTOS_ERR_FAIL       Audio device not successfully initialized.
*
* Return(s)   : None.
*
* Note(s)     : None.
                                     ...
                                     RTOS_ERR        *p_err)
{
    ...
    PP_UNUSED_PARAM(p_audio_drv);

                                                                /* Set default values.                                  */
    Ex_USBD_Audio_DrvData.MicInfo.AS_Handle    =  USBD_AUDIO_DRV_SIMULATION_AS_HANDLE_NONE;
    Ex_USBD_Audio_DrvData.MicInfo.Vol          =  USBD_AUDIO_DRV_SIMULATION_VOLUME_MAX;
    Ex_USBD_Audio_DrvData.MicInfo.Mute         =  DEF_FALSE;
    Ex_USBD_Audio_DrvData.MicInfo.Gain         =  ConvTbl_dB_ToGain[0u];
    Ex_USBD_Audio_DrvData.MicInfo.SamFreq      =  USBD_AUDIO_FMT_TYPE_I_SAMFREQ_44_1KHZ;
    Ex_USBD_Audio_DrvData.MicInfo.BufHeadPtr   =  Ex_USBD_Audio_DrvSimulationDataTbl44_1;
    Ex_USBD_Audio_DrvData.MicInfo.BufNbSamples =  Ex_USBD_Audio_DrvSimulationDataNbSamples44_1;

    Mem_Clr((void *)&Ex_USBD_Audio_DrvCircularBufRecord,
                     sizeof(Ex_USBD_Audio_DrvCircularBufRecord));

                                                                /* Create task.                                         */
    p_stk = Mem_SegAllocExt("Audio Mic Driver task stack",
                             DEF_NULL,
                             EX_USBD_AUDIO_DRV_SIMULATION_MIC_TASK_STK_SIZE * sizeof(CPU_STK),
    ...
    OSTaskCreate(&USBD_Audio_DrvSimulationMicTaskTCB,
                 "Audio Mic Driver Simulation",
                  USBD_Audio_DrvSimulationMicTask,
                  DEF_NULL,
                  EX_USBD_AUDIO_DRV_SIMULATION_MIC_TASK_PRIO,
                  p_stk,
                  EX_USBD_AUDIO_DRV_SIMULATION_MIC_TASK_STK_SIZE / 10u,
                  EX_USBD_AUDIO_DRV_SIMULATION_MIC_TASK_STK_SIZE,
                  0u,
                  0u,
                  DEF_NULL,
                                      ...
                                      RTOS_ERR        *p_err)
{
    ...
    PP_UNUSED_PARAM(p_audio_drv);
                                                                ...
                                                                /* Set default values.                                  */
    Ex_USBD_Audio_DrvData.SpeakerInfo.AS_Handle = USBD_AUDIO_DRV_SIMULATION_AS_HANDLE_NONE;
    Ex_USBD_Audio_DrvData.SpeakerInfo.Vol       = USBD_AUDIO_DRV_SIMULATION_VOLUME_MAX;
    Ex_USBD_Audio_DrvData.SpeakerInfo.Mute      = DEF_FALSE;
    Ex_USBD_Audio_DrvData.SpeakerInfo.Gain      = ConvTbl_dB_ToGain[0u];

    Mem_Clr((void *)&Ex_USBD_Audio_DrvCircularBufPlayback,
                     sizeof(Ex_USBD_Audio_DrvCircularBufPlayback));
    ...
    p_stk = Mem_SegAllocExt("Audio Mic Driver task stack",
                             DEF_NULL,
                             EX_USBD_AUDIO_DRV_SIMULATION_LOOP_TASK_STK_SIZE * sizeof(CPU_STK),
                             CPU_CFG_STK_ALIGN_BYTES,
                             DEF_NULL,
                             p_err);
    if (p_err->Code != RTOS_ERR_NONE) {
    ...
    OSTaskCreate(&USBD_Audio_DrvSimulationLoopTaskTCB,
                 "Audio Loop Driver Simulation",
                  USBD_Audio_DrvSimulationLoopTask,
                  DEF_NULL,
                  EX_USBD_AUDIO_DRV_SIMULATION_LOOP_TASK_PRIO,
                  p_stk,
                  EX_USBD_AUDIO_DRV_SIMULATION_LOOP_TASK_STK_SIZE / 10u,
                  EX_USBD_AUDIO_DRV_SIMULATION_LOOP_TASK_STK_SIZE,
                  0u,
                  0u,
                  DEF_NULL,
                  OS_OPT_TASK_STK_CHK | OS_OPT_TASK_STK_CLR,
                                                      ...
                                                      CPU_BOOLEAN     *p_mute)
{
    ...
    if (unit_id == Ex_USBD_AudioMic_FU_ID) {

        if (set_en == DEF_FALSE) {                              /* Get.                                                 */
            CPU_CRITICAL_ENTER();
           *p_mute = Ex_USBD_Audio_DrvData.MicInfo.Mute;
            CPU_CRITICAL_EXIT();
            req_ok = DEF_OK;
        } else {                                                /* Set.                                                 */
            CPU_CRITICAL_ENTER();
            Ex_USBD_Audio_DrvData.MicInfo.Mute = *p_mute;
            CPU_CRITICAL_EXIT();
            req_ok = DEF_OK;
        }
    } else if (unit_id == Ex_USBD_AudioSpeaker_FU_ID) {

        if (set_en == DEF_FALSE) {                              /* Get.                                                 */
            CPU_CRITICAL_ENTER();
           *p_mute = Ex_USBD_Audio_DrvData.SpeakerInfo.Mute;
            CPU_CRITICAL_EXIT();
            req_ok = DEF_OK;
        } else {                                                /* Set.                                                 */
            CPU_CRITICAL_ENTER();
            Ex_USBD_Audio_DrvData.SpeakerInfo.Mute = *p_mute;
            CPU_CRITICAL_EXIT();
            req_ok = DEF_OK;
        }
    }
                                                     ...
                                                     CPU_INT16U      *p_vol)
{
    ...
    switch (req) {
        case USBD_AUDIO_REQ_GET_CUR:
             if (unit_id == Ex_USBD_AudioMic_FU_ID){                         /* Verify unit ID.                                      */
                 CPU_CRITICAL_ENTER();
                *p_vol = Ex_USBD_Audio_DrvData.MicInfo.Vol;
                 CPU_CRITICAL_EXIT();
                 req_ok = DEF_OK;
             } else if (unit_id == Ex_USBD_AudioSpeaker_FU_ID){              /* Verify unit ID.                                      */
                 CPU_CRITICAL_ENTER();
                *p_vol = Ex_USBD_Audio_DrvData.SpeakerInfo.Vol;
                 CPU_CRITICAL_EXIT();
                 req_ok = DEF_OK;
             }
             break;
        ...
        case USBD_AUDIO_REQ_SET_CUR:
             if (unit_id == Ex_USBD_AudioMic_FU_ID){                         /* Verify unit ID.                                      */
                 p_fu_unit_info = &Ex_USBD_Audio_DrvData.MicInfo;
             } else if (unit_id == Ex_USBD_AudioSpeaker_FU_ID) {             /* Verify unit ID.                                      */
                 p_fu_unit_info = &Ex_USBD_Audio_DrvData.SpeakerInfo;
             } else {
                 break;
             }
                                                             ...
                                                             CPU_INT32U      *p_sampling_freq)
{
    ...
    if (set_en == DEF_FALSE) {
        CPU_CRITICAL_ENTER();
       *p_sampling_freq = Ex_USBD_Audio_DrvData.MicInfo.SamFreq;
        CPU_CRITICAL_EXIT();
        req_ok          = DEF_OK;
                                                                /* ----------------------- SET ------------------------ */
    } else {
        ...
        if ((*p_sampling_freq == USBD_AUDIO_FMT_TYPE_I_SAMFREQ_44_1KHZ) ||
            ...
            CPU_CRITICAL_ENTER();
            Ex_USBD_Audio_DrvData.MicInfo.SamFreq = *p_sampling_freq;
            if (*p_sampling_freq == USBD_AUDIO_FMT_TYPE_I_SAMFREQ_44_1KHZ) {
                Ex_USBD_Audio_DrvData.MicInfo.BufHeadPtr   = Ex_USBD_Audio_DrvSimulationDataTbl44_1;
                Ex_USBD_Audio_DrvData.MicInfo.BufNbSamples = Ex_USBD_Audio_DrvSimulationDataNbSamples44_1;
            } else {
                Ex_USBD_Audio_DrvData.MicInfo.BufHeadPtr   = Ex_USBD_Audio_DrvSimulationDataTbl48;
                Ex_USBD_Audio_DrvData.MicInfo.BufNbSamples = Ex_USBD_Audio_DrvSimulationDataNbSamples48;
            }
            CPU_CRITICAL_EXIT();
                                                ...
                                                CPU_INT08U             terminal_id_link)
{
    ...
    if (terminal_id_link == Ex_USBD_AudioMic_OT_USB_IN_ID) {
        CPU_CRITICAL_ENTER();
        Ex_USBD_Audio_DrvData.MicInfo.AS_Handle = as_handle;
        CPU_CRITICAL_EXIT();
        req_ok = DEF_OK;
    } else if (terminal_id_link == Ex_USBD_AudioSpeaker_IT_USB_OUT_ID) {
        CPU_INT08U  i;


        CPU_CRITICAL_ENTER();
        Ex_USBD_Audio_DrvData.SpeakerInfo.AS_Handle = as_handle;
        CPU_CRITICAL_EXIT();
...
*
*                                   RTOS_ERR_NONE   Buffer successfully retrieved.
*                                   RTOS_ERR_RX     Generic Rx error.
*
* Return(s)   : None.
*
* Note(s)     : None.
                                            ...
                                            RTOS_ERR        *p_err)
{
    ...
    if (terminal_id_link != Ex_USBD_AudioMic_OT_USB_IN_ID) {
        p_err->Code = RTOS_ERR_RX;
        return;
    }
                                                                /* Get ready record buf.                                */
    p_buf_local = USBD_Audio_DrvSimulation_CircularBufStreamGet(&Ex_USBD_Audio_DrvCircularBufRecord, p_buf_len);
                                              ...
                                              RTOS_ERR        *p_err)
{
    ...
    if (terminal_id_link != Ex_USBD_AudioSpeaker_IT_USB_OUT_ID) {
        p_err->Code = RTOS_ERR_TX;
        return;
    }
                                                                /* Store ready playback buf.                            */
    result = USBD_Audio_DrvSimulation_CircularBufStreamStore(             &Ex_USBD_Audio_DrvCircularBufPlayback,
                                                             (CPU_INT08U *)p_buf,
                                                                           buf_len);
...
static  void  USBD_Audio_DrvSimulationLoopTask(void  *p_arg)
{
    CPU_INT16S             *p_circ_buf           = &Ex_USBD_Audio_DrvCircularBufLoopback.Buf[0u];
    ...
    Ex_USBD_Audio_DrvCircularBufLoopback.IxIn     =  0u;
    Ex_USBD_Audio_DrvCircularBufLoopback.IxOut    =  0u;
    Ex_USBD_Audio_DrvCircularBufLoopback.NbrXfers =  0u;

    while (DEF_TRUE) {                                          /* Task body.                                           */
                                                                /* -------- COPY PROTECTED VARS TO LOCAL VARS --------- */
        CPU_CRITICAL_ENTER();
        speaker_as_handle = Ex_USBD_Audio_DrvData.SpeakerInfo.AS_Handle;
        speaker_fu_vol    = Ex_USBD_Audio_DrvData.SpeakerInfo.Vol;
        speaker_fu_mute   = Ex_USBD_Audio_DrvData.SpeakerInfo.Mute;
        speaker_fu_gain   = Ex_USBD_Audio_DrvData.SpeakerInfo.Gain;

        mic_as_handle = Ex_USBD_Audio_DrvData.MicInfo.AS_Handle;
        mic_fu_vol    = Ex_USBD_Audio_DrvData.MicInfo.Vol;
        mic_fu_mute   = Ex_USBD_Audio_DrvData.MicInfo.Mute;
        mic_fu_gain   = Ex_USBD_Audio_DrvData.MicInfo.Gain;
        CPU_CRITICAL_EXIT();
        ...
        if (speaker_as_handle != USBD_AUDIO_DRV_SIMULATION_AS_HANDLE_NONE) {
            ...
            avail_space = USBD_AUDIO_DRV_SIMULATION_LOOP_CIRCULAR_BUF_LEN - Ex_USBD_Audio_DrvCircularBufLoopback.BufCurCnt;
                                                                /* Make sure circular buf can contain another xfer.     */
            if (avail_space >= USBD_AUDIO_DRV_SIMULATION_DATA_XFER_SAMPLES) {
                                                                /* Get an audio buf.                                    */
                p_buf_xfer = (CPU_INT16S *)USBD_Audio_DrvSimulation_CircularBufStreamGet(&Ex_USBD_Audio_DrvCircularBufPlayback, &buf_len);
                ...
                if (p_buf_xfer != DEF_NULL) {
                    ...
                    if ((speaker_fu_vol  == USBD_AUDIO_DRV_SIMULATION_VOLUME_MUTE) ||
                        ...
                        for (ix = 0u; ix < tbl_len; ++ix) {
                            p_circ_buf[Ex_USBD_Audio_DrvCircularBufLoopback.IxIn] = 0;
                            Ex_USBD_Audio_DrvCircularBufLoopback.IxIn             = (Ex_USBD_Audio_DrvCircularBufLoopback.IxIn + 1u) % USBD_AUDIO_DRV_SIMULATION_LOOP_CIRCULAR_BUF_LEN;
                        }
                    } else {
                        for (ix = 0u; ix < tbl_len; ++ix) {     /* Fill audio buf with specified sample.                */
                            ...
                            p_circ_buf[Ex_USBD_Audio_DrvCircularBufLoopback.IxIn] = (CPU_INT16S)calc_val;
                                                                /* Adjust circular buf ix.                              */
                            Ex_USBD_Audio_DrvCircularBufLoopback.IxIn             = (Ex_USBD_Audio_DrvCircularBufLoopback.IxIn + 1u) % USBD_AUDIO_DRV_SIMULATION_LOOP_CIRCULAR_BUF_LEN;
                        }
                    }

                    Ex_USBD_Audio_DrvCircularBufLoopback.BufCurCnt += tbl_len;   /* Track cur nbr of samples in circular buf.            */
                    ...
                    USBD_Audio_PlaybackTxCmpl(speaker_as_handle);
                                                                ...
                                                                /* See Note #2.                                         */
        } else if (Ex_USBD_Audio_DrvCircularBufLoopback.BufCurCnt < USBD_AUDIO_DRV_SIMULATION_DATA_XFER_SAMPLES) {
            speaker_priming_done = DEF_NO;
        ...
        if ((mic_as_handle        != USBD_AUDIO_DRV_SIMULATION_AS_HANDLE_NONE) &&
            ...
            if (Ex_USBD_Audio_DrvCircularBufLoopback.BufCurCnt >= USBD_AUDIO_DRV_SIMULATION_RECORD_DATA_XFER_SAMPLES) {
                                                                /* Get an empty buf.                                    */
                ...
                if (p_buf_xfer != DEF_NULL) {
                    ...
                    if ((mic_fu_vol  == USBD_AUDIO_DRV_SIMULATION_VOLUME_MUTE) ||
                        ...
                        Mem_Clr((void *)&p_buf_xfer[0u],
                                         buf_len);
                        Ex_USBD_Audio_DrvCircularBufLoopback.IxOut = (Ex_USBD_Audio_DrvCircularBufLoopback.IxOut + (tbl_len)) % USBD_AUDIO_DRV_SIMULATION_LOOP_CIRCULAR_BUF_LEN;

                    } else {
                        for (ix = 0u; ix < tbl_len; ix++) {
                                                                /* Apply linear gain on each sample (see Note #1).      */
                            calc_val                  = (CPU_INT32S)p_circ_buf[Ex_USBD_Audio_DrvCircularBufLoopback.IxOut];
                                                                /* Multiply by linear gain value (from look-up tbl).    */
                            ...
                            p_buf_xfer[ix]            = (CPU_INT16S)calc_val;
                                                                /* Adjust circular buf ix.                              */
                            Ex_USBD_Audio_DrvCircularBufLoopback.IxOut = (Ex_USBD_Audio_DrvCircularBufLoopback.IxOut + 1u) % USBD_AUDIO_DRV_SIMULATION_LOOP_CIRCULAR_BUF_LEN;
                        }
                    }
                    ...
                    result = USBD_Audio_DrvSimulation_CircularBufStreamStore(             &Ex_USBD_Audio_DrvCircularBufRecord,
...
static  void  USBD_Audio_DrvSimulationMicTask (void  *p_arg)
{
   ...
   (void)&p_arg;
    ...
    while (DEF_TRUE) {                                          /* Task body.                                           */
        ...
        CPU_CRITICAL_ENTER();
        as_handle       = Ex_USBD_Audio_DrvData.MicInfo.AS_Handle;       /* Copy protected vars to local vars.                   */
        fu_vol          = Ex_USBD_Audio_DrvData.MicInfo.Vol;
        fu_mute         = Ex_USBD_Audio_DrvData.MicInfo.Mute;
        fu_gain         = Ex_USBD_Audio_DrvData.MicInfo.Gain;
        p_data_buf      = Ex_USBD_Audio_DrvData.MicInfo.BufHeadPtr;
        nb_data_samples = Ex_USBD_Audio_DrvData.MicInfo.BufNbSamples;
        CPU_CRITICAL_EXIT();
