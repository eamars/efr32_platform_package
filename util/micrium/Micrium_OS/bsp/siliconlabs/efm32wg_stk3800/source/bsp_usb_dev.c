...
*/
                                                                ...
                                                                /* ----------------- USB DEVICE FNCTS ----------------- */
static  void  BSP_USBD_EFM32WG_STK3800_Init   (USBD_DRV  *p_drv);

static  void  BSP_USBD_EFM32WG_STK3800_Conn   (void);

static  void  BSP_USBD_EFM32WG_STK3800_Disconn(void);

static  void  BSP_USBD_EFM32WG_IntHandler     (void);
...
static  USBD_DRV  *BSP_USBD_EFM32WG_DrvPtr;                     /* USB device driver specific info.                     */


                                                                /* ---------- USB DEVICE ENDPOINTS INFO TBL ----------- */
static  USBD_DRV_EP_INFO  BSP_USBD_EFM32WG_EP_InfoTbl[] = {
...
static  USBD_DRV_INFO  BSP_USBD_EFM32WG_DrvInfoPtr = {
    .BaseAddr   = 0x40100000u,
    .MemAddr    = 0x00000000u,
    .MemSize    = 0u,
    .Spd        = USBD_DEV_SPD_FULL,
    .EP_InfoTbl = BSP_USBD_EFM32WG_EP_InfoTbl
};

                                                                /* ------------- USB DEVICE BSP STRUCTURE ------------- */
static  USBD_DRV_BSP_API   BSP_USBD_EFM32WG_STK3800_BSP_API_Ptr = {
    BSP_USBD_EFM32WG_STK3800_Init,
    BSP_USBD_EFM32WG_STK3800_Conn,
    BSP_USBD_EFM32WG_STK3800_Disconn
};

                                                                /* ----------- USB DEVICE HW INFO STRUCTURE ----------- */
const  USBD_DEV_CTRLR_HW_INFO  BSP_USBD_EFM32WG_HwInfo = {
    .DrvAPI_Ptr  = &USBD_DrvAPI_EFM32_OTG_FS,
    .DrvInfoPtr  = &BSP_USBD_EFM32WG_DrvInfoPtr,
    .BSP_API_Ptr = &BSP_USBD_EFM32WG_STK3800_BSP_API_Ptr
};
