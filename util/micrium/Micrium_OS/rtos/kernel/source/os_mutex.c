...
*                                            OSMutexCreate()
*
* Description : Creates a mutex so that multiple program threads can take turns sharing the same resource.
*
* Argument(s) : p_mutex     Pointer to the mutex to initialize. Your application is responsible for
*                           allocating storage for the mutex.
*
*               p_name      Pointer to the name you would like to give the mutex.
*
...
*                                             OSMutexDel()
*
* Description : Deletes a mutex and readies all tasks pending on the mutex.
*
* Argument(s) : p_mutex     Pointer to the mutex to delete.
*
...
*
* Return(s)   : == 0    If there are no tasks waiting on the mutex, or upon error.
*               >  0    If there are one or more tasks waiting on the mutex that are now readied
*                       and informed.
*
* Note(s)     : (1) Use this function with care. Tasks that would normally expect the presence of the
*                   mutex MUST check the return code of OSMutexPend().
*
*               (2) Because ALL tasks pending on the mutex will be readied, be careful in applications
...
*
* Argument(s) : p_mutex     Pointer to the mutex.
*
*               timeout     Optional timeout period (in clock ticks). If non-zero, the task will wait
...
*
*               opt         Determines whether the feature to block if the mutex is available or not:
*
*                               OS_OPT_PEND_BLOCKING        Task will     block.
*                               OS_OPT_PEND_NON_BLOCKING    Task will NOT block.
*
*               p_ts        Pointer to a variable that will receive the timestamp of when the mutex
...
*                                          OSMutexPendAbort()
*
* Description : Aborts and readies any tasks currently waiting on a mutex. Rather than signal the mutex
...
*
* Return(s)   : == 0    If there were no tasks waiting on the mutex, or upon error.
...
*                                             OSMutexPost()
*
* Description : Signals a mutex.
*
* Argument(s) : p_mutex     Pointer to the mutex.
*
*               opt         Option that alters the behavior of the post. The choices are:
*
...
*
* Argument(s) : p_mutex     Pointer to the mutex to clear.
*
* Return(s)   : None.
*
...
*                                         OS_MutexDbgListAdd()
*
* Description : Add a mutex to the mutex debug list.
*
* Argument(s) : p_mutex     Pointer to the mutex to add.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*
* Description : Remove a mutex from the mutex debug list.
*
* Argument(s) : p_mutex     Pointer to the mutex to remove.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*
* Argument(s) : p_tcb       Pointer to the tcb of the task to give the mutex to.
*
*               p_mutex     Pointer to the mutex to add to the group.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*
* Argument(s) : p_tcb       Pointer to the tcb of the task to remove the mutex from.
*
*               p_mutex     Pointer to the mutex to remove from the group.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*
* Description : This function is called by the kernel to find the highest task pending on any mutex from
*               a group.
*
* Argument(s) : p_tcb   Pointer to the TCB of the task to process.
*
* Return(s)   : Highest priority pending or OS_CFG_PRIO_MAX - 1u if none found.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
*********************************************************************************************************
*/
...
*                                         OS_MutexGrpPostAll()
*
* Description : This function is called by the kernel to post (release) all the mutex from a group. Used
