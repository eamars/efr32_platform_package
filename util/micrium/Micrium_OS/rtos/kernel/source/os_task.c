...
*                                          OSTaskChangePrio()
*
* Description : Allows you to dynamically change the priority of a task. Note that the new
*               priority MUST be available.
*
* Argument(s) : p_tcb       Pointer to the TCB of the task for which to change the priority.
*
*               prio_new    The new priority.
*
...
*                                            OSTaskCreate()
*
* Description : Allows the Kernel to manage the execution of a task. Tasks can either
*               be created prior to the start of multitasking or by a running task. A task cannot be
*               created by an ISR.
*
* Argument(s) : p_tcb           Pointer to the task's TCB.
*
*               p_name          Pointer to an ASCII string that provides a name for the task.
*
*               p_task          Pointer to the task's code.
*
*               p_arg           Pointer to an optional data area which can pass parameters to the task
...
*                                   }
*
*               prio            The task's priority. A unique priority MUST be assigned to each task.
*                               The lower the number, the higher the priority.
*
*               p_stk_base      Pointer to the base address of the stack (i.e. low address).
*
*               stk_limit       The number of stack elements to set as 'watermark' limits for the stack.
...
*
*               stk_size        The size of the stack in number of elements. If CPU_STK is set to
...
*
*               q_size          The maximum number of messages that can be sent to the task.
*
*               time_quanta     Amount of time (in ticks) for a time slice when the round-robin between
*                               tasks. Set to 0 to use the default.
*
*               p_ext           Pointer to a user-supplied memory location which is used as a TCB
...
* Return(s)   : None.
*
* Note(s)     : (1) OSTaskCreate() triggers a critical assert when a stack overflow is detected during
...
*                                              OSTaskDel()
*
* Description : Allows you to delete a task. The calling task can delete itself by specifying a NULL
...
*
* Argument(s) : p_tcb   Pointer to the TCB of the task to delete.
*
*               p_err   Pointer to the variable that will receive one of the following error code(s)
*                       from this function:
*
...
* Return(s)   : None.
*
* Note(s)     : (1) 'p_err' is set to RTOS_ERR_NONE before OSSched() to allow the returned err or code
...
*                                            OSTaskQFlush()
*
* Description : Flushes the task's internal message queue.
*
* Argument(s) : p_tcb   Pointer to the task's TCB. Specifying a NULL pointer indicates that
...
*                           RTOS_ERR_NONE
*
* Return(s)   : The number of entries freed from the queue.
*
* Note(s)     : (1) Use this function with great care. When you flush the queue, you lose the references
...
*
* Argument(s) : timeout     Optional timeout period (in clock ticks). If non-zero, your task will
...
*                           message arrives.
*
*               opt         Determines if the user wants to block if the task's queue is empty or
*                           not:
*
*                               OS_OPT_PEND_BLOCKING        Task will     block.
*                               OS_OPT_PEND_NON_BLOCKING    Task will NOT block.
*
*               p_msg_size  Pointer to a variable that will receive the size of the message.
*
*               p_ts        Pointer to a variable that will receive the timestamp of when the
...
*                               RTOS_ERR_TIMEOUT
*
* Return(s)   : A pointer to the message received or a NULL pointer upon error.
*
* Note(s)     : It is possible to receive a NULL pointer when there are no errors.
*********************************************************************************************************
*/
...
*                                          OSTaskQPendAbort()
*
* Description : Aborts and readies the task specified. Use this function to fault-abort the wait for a
*               message, rather than to normally post the message to the task via OSTaskQPost().
*
*
* Argument(s) : p_tcb   Pointer to the TCB of the task to pend abort.
*
...
*
* Return(s)   : == DEF_FALSE    If task was not waiting for a message, or upon error.
...
*                                             OSTaskQPost()
*
* Description : Sends a message to a task.
*
* Argument(s) : p_tcb       Pointer to the TCB of the task receiving a message. If you specify a NULL
...
*
*               p_void      Pointer to the message to send.
*
*               msg_size    The size of the message sent (in bytes).
*
...
*                                            OSTaskRegGet()
*
* Description : Obtains the current value of a task register. Task registers are application specific
*               and can be used to store task specific values such as 'error numbers' (i.e. errno),
*               statistics, etc.
*
* Argument(s) : p_tcb   Pointer to the TCB of the task from which you want to read the register.
*                       If 'p_tcb' is a NULL pointer, you will get the register of the current task.
*
*               id      The 'id' of the desired task variable. Note that the 'id' must be less than
...
*                           RTOS_ERR_NONE
*
* Return(s)   : The current value of the task's register, or 0 if an error is detected.
*
* Note(s)     : None.
*********************************************************************************************************
*/
...
*                                           OSTaskRegGetID()
*
* Description : This function obtains a task register ID. This function allows task register IDs
*               to be allocated dynamically instead of statically.
*
* Argument(s) : p_err   Pointer to the variable that will receive one of the following error code(s)
*                       from this function:
*
*                           RTOS_ERR_NONE
*                           RTOS_ERR_NO_MORE_RSRC
*
* Return(s)   : The next available task register 'id' or OS_CFG_TASK_REG_TBL_SIZE if an error is detected.
*
* Note(s)     : None.
*********************************************************************************************************
*/
...
*                                            OSTaskRegSet()
*
* Description : Changes the current value of a task register. Task registers are application specific
*               and can be used to store task specific values such as error numbers (i.e. errno),
*               statistics, etc.
*
* Argument(s) : p_tcb   Pointer to the TCB of the task for which you want to set the register.
*                       If 'p_tcb' is a NULL pointer, change the register of the current task.
*
*               id      The 'id' of the desired task register. Note that the 'id' must be less than
*                       OS_CFG_TASK_REG_TBL_SIZE.
*
*               value   The desired value for the task register.
*
*               p_err   Pointer to the variable that will receive one of the following error code(s)
*                       from this function:
*
...
*                                            OSTaskResume()
*
* Description : Resumes a previously suspended task. This is the only call that removes an explicit
*               task suspension.
*
* Argument(s) : p_tcb   Pointer to the TCB of the task to resume.
*
*               p_err   Pointer to the variable that will receive one of the following error code(s)
*                       from this function:
*
...
*                                            OSTaskSemPend()
*
* Description : Blocks the current task until a signal is sent by another task or ISR.
*
*
* Argument(s) : timeout     The amount of time you will wait for the signal.
*
*               opt         Determines if the user wants to block if a semaphore post was not received:
*
*                               OS_OPT_PEND_BLOCKING        Task will     block.
*                               OS_OPT_PEND_NON_BLOCKING    Task will NOT block.
*
...
*                               RTOS_ERR_TIMEOUT
*
* Return(s)   : The current count of signals the task received, 0 if none.
*
* Note(s)     : None.
*********************************************************************************************************
*/
...
*                                         OSTaskSemPendAbort()
*
* Description : Aborts and readies the task specified. This function should be used to
...
*                                            OSTaskSemPost()
*
* Description : Signals a task waiting for a signal.
*
* Argument(s) : p_tcb   The pointer to the TCB of the task to signal. A NULL pointer indicates that
...
*                           RTOS_ERR_INVALID_STATE
*
* Return(s)   : The current value of the task's signal counter, or 0 if called from an ISR.
*
* Note(s)     : None.
*********************************************************************************************************
*/
...
*                                            OSTaskSemSet()
*
* Description : Clears the signal counter.
*
* Argument(s) : p_tcb   Pointer to the TCB of the task to clear the counter. If you specify a
*                       NULL pointer, the signal counter of the current task will be cleared.
*
*               cnt     The desired value of the semaphore counter.
*
*               p_err   Pointer to the variable that will receive one of the following error code(s)
*                       from this function:
*
...
*
* Return(s)   : The value of the signal counter before being set, or 0 on error.
*
* Note(s)     : None.
*********************************************************************************************************
*/
...
*                                            OSTaskStkChk()
*
* Description : Calculates the amount of free memory left on the specified task's stack.
*
* Argument(s) : p_tcb   Pointer to the TCB of the task to check. If you specify a NULL pointer,
*                       you are specifying that you want to check the stack of the current task.
*
*               p_free  Pointer to a variable that will receive the number of free 'entries' on
*                       the task's stack.
*
*               p_used  Pointer to a variable that will receive the number of used 'entries' on
*                       the task's stack.
*
*               p_err   Pointer to the variable that will receive one of the following error code(s)
*                       from this function:
...
*                                         OSTaskStkRedzoneChk()
*
* Description : Verifies a task's stack redzone.
*
* Argument(s) : p_tcb   Pointer to the TCB of the task to check or null for the current task.
*
* Return(s)   : == DEF_FAIL     If the stack is     corrupted.
...
*                                            OSTaskSuspend()
*
* Description : This function is called to suspend a task. The task can be the calling task if 'p_tcb' is
...
* Return(s)   : None.
*
* Note(s)     : Use this function with great care. If you suspend a task that is waiting for an event
*               (i.e. a message, a semaphore, a queue, etc.) you will prevent this task from running
...
*                                         OSTaskTimeQuantaSet()
*
* Description : Changes the value of the task's specific time slice.
*
* Argument(s) : p_tcb           Pointer to the TCB of the task to change. If you specify an NULL
*                               pointer, the current task is assumed.
*
*               time_quanta     The number of ticks before the CPU is taken away when round-robin
...
*                                          OS_TaskDbgListAdd()
*
* Description : Add a task to the task debug list.
*
* Argument(s) : p_tcb   Pointer to the TCB to add.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*
* Description : Remove a task from the task debug list.
*
* Argument(s) : p_tcb   Pointer to the TCB to remove.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*
* Argument(s) : p_err   Pointer to the variable that will receive one of the following error code(s) from this function:
*
*                           RTOS_ERR_NONE
*
* Return(s)   : None.
...
*
* Argument(s) : p_tcb   Pointer to the TCB to initialize.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*
* Argument(s) : p_base      Pointer to the base of the stack.
*
*               stk_size    The size of the stack.
*
* Return(s)   : == DEF_FAIL     If the stack is     corrupted.
*               == DEF_OK       If the stack is NOT corrupted.
*
...
*                                          OS_TaskChangePrio()
*
* Description : This function is called by the kernel to perform the actual operation of changing a task's
*               priority. Priority inheritance is updated if necessary.
*
* Argument(s) : p_tcb       Pointer to the TCB of the task to change the priority.
*
*               prio_new    The new priority to give to the task.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
