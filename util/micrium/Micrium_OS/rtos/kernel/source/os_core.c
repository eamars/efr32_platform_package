...
#if (OS_CFG_COMPAT_INIT_EN == DEF_DISABLED)
...
*                                         OS_ConfigureISRStk()
*
* Description : Configure the stack used for ISRs, if available.
*
* Argument(s) : p_stk_base_ptr  Pointer to the base of the buffer used as the stack.
*
*               stk_size        Size of the stack, in CPU_STK elements.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is optional. If it is called, it must be called before OSInit().
...
*                                         OS_ConfigureMemSeg()
*
* Description : Configure the memory segment used by the kernel.
*
* Argument(s) : p_mem_seg   Pointer to the memory segment in which the kernel data will be allocated.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is optional. If it is called, it must be called before OSInit().
...
*
* Description : Configure the kernel message pool size.
*
* Argument(s) : msg_pool_size   Number of messages the kernel will manage. Shared between task message
...
*
* Description : Configure the application stack limit.
*
* Argument(s) : task_stk_limit  Stack limit in percentage to empty.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is optional. If it is called, it must be called before OSInit().
...
*
* Description : Configure the stack used by the Idle Task.
*
* Argument(s) : p_stk_base_ptr  Pointer to the base of the buffer used as the stack.
*
*               stk_size        Size of the stack, in CPU_STK elements.
...
*
* Description : If enabled, configure the Statistics Task.
*
* Argument(s) : p_stat_task_cfg     Pointer to the Statistics Task configuration.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is optional. If it is called, it must be called before OSInit().
...
*
* Description : If enabled, configure the Tick Task.
*
* Argument(s) : p_tick_task_cfg     Pointer to the Tick Task configuration.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is optional. If it is called, it must be called before OSInit().
...
*                                         OS_ConfigureTmrTask()
*
* Description : If enabled, configure the Timer Management Task.
*
* Argument(s) : p_tmr_task_cfg  Pointer to the Timer Management Task configuration.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is optional. If it is called, it must be called before OSInit().
...
*                                               OSInit()
*
* Description : Initializes the internals of the Kernel and MUST be called before creating any Kernel
...
* Return(s)   : None.
*
* Note(s)     : (1) This function MUST be called AFTER Common's Mem_Init().
*********************************************************************************************************
*/
...
*                                             OSIntEnter()
*
* Description : Used in an interrupt service routine (ISR) to notify the Kernel that you are about to
...
*
*               (2) Your ISR can directly increment 'OSIntNestingCtr' without calling this function because
*                   OSIntNestingCtr has been declared 'global'. The port is actually considered part of the
*                   OS and is allowed to access the Kernel's variables.
*
*               (3) You MUST still call OSIntExit() even though you can increment 'OSIntNestingCtr' directly.
*
*               (4) You MUST invoke OSIntEnter() and OSIntExit() in pairs. In other words, for every call
...
*                                              OSIntExit()
*
* Description : Notifies the Kernel that you have completed servicing an ISR. When the last nested ISR
...
* Return(s)   : None.
*
* Note(s)     : (1) You MUST invoke OSIntEnter() and OSIntExit() in pairs. In other words, for every call
*                   to OSIntEnter() (or direct increment to OSIntNestingCtr) at the beginning of the ISR,
*                   you MUST have a call to OSIntExit() at the end of the ISR.
*
...
*                                               OSSched()
*
* Description : This function is called by other Kernel services to determine whether a new, high
...
*                                             OSSchedLock()
*
* Description : Prevents rescheduling from taking place, allowing your application to prevent context
...
* Return(s)   : None.
*
* Note(s)     : (1) You MUST invoke OSSchedLock() and OSSchedUnlock() in pairs. In other words, for every
...
*                                            OSSchedUnlock()
*
* Description : Re-allows rescheduling.
*
* Argument(s) : p_err   Pointer to the variable that will receive one of the following error code(s)
*                       from this function:
*
...
*
* Description : Changes the round-robin scheduling parameters.
*
* Argument(s) : en                  Determines if the round-robin will be used:
*
*                                       DEF_ENABLED     Round-robin scheduling is enabled.
*                                       DEF_DISABLED    Round-robin scheduling is disabled.
*
*               dflt_time_quanta    Default number of ticks between time slices.
...
*
* Description : Gives up the CPU when a task is finished its execution before its time slice expires.
*
* Argument(s) : p_err   Pointer to the variable that will receive one of the following error code(s)
*                       from this function:
*
...
*                                               OSStart()
*
* Description : Starts the multitasking process which lets the Kernel manage the tasks that you created.
...
*                                             OS_IdleTask()
*
* Description : This task is internal to the Kernel and executes whenever no other higher priority tasks
*               execute because they are ALL waiting for event(s) to occur.
*
* Argument(s) : p_arg   Argument passed to the task when the task is created.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
*
*               (2) OSIdleTaskHook() is called after the critical section to ensure that interrupts will
...
*
*               (3) This hook has been added to allow you to do such things as STOP the CPU to reduce
*                   power usage.
*********************************************************************************************************
*/
...
*                                               OS_Pend()
*
* Description : This function is called to place a task in the blocked state waiting for an event to occur.
*               This function exists because it is common to a number of OSxxxPend() services.
*
* Argument(s) : p_obj       Pointer to the object to pend on. If there are no object used to pend
...
*                                                              the task)
*
*               timeout     Amount of time the task will wait for the event to occur.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*                                            OS_PendAbort()
*
* Description : This function is called by the OSxxxPendAbort() and OSxxxDel() functions to cancel pending
*               on an event.
*
* Argument(s) : p_tcb   Pointer to the OS_TCB of the task that we'll abort the pend for.
*
*               ts      Timestamp as to when the pend was cancelled.
*
...
*                                          OS_PendDbgNameAdd()
*
* Description : Add pointers to ASCII 'names' of objects so they can easily be displayed using a kernel
*               aware tool.
*
* Argument(s) : p_obj   Pointer to the object being pended on.
*
*               p_tcb   Pointer to the OS_TCB of the task pending on the object.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*
* Description : Remove pointers to ASCII 'names' of objects so they can easily be displayed using a kernel
*               aware tool.
*
* Argument(s) : p_obj   Pointer to the object being pended on.
*
...
*
* Description : This function is called to change the position of a task waiting in a pend list. The
*               strategy used is to remove the task from the pend list and add it again using its changed
*               priority.
*
* Argument(s) : p_tcb   Pointer to the TCB of the task to move.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*
* Argument(s) : p_pend_list     Pointer to an OS_PEND_LIST.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*
* Description : This function is called to place an OS_TCB entry in a linked list based on its priority.
...
*                                         +--------------+     +--------------+      +--------------+
*
* Argument(s) : p_pend_list     Pointer to the OS_PEND_LIST where the OS_TCB entry will be inserted.
*
*               p_tcb           The OS_TCB to insert in the list.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*                                          OS_PendListRemove()
*
* Description : This function is called to remove a task from a pend list knowing its TCB.
*
...
*                                         +--------------+     +--------------+      +--------------+
*
* Argument(s) : p_tcb   Pointer to the TCB of the task to remove from the pend list.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*                                               OS_Post()
*
* Description : This function is called to post to a task. This function exist because it is common to a
*               number of OSxxxPost() services.
*
* Argument(s) : p_obj       Pointer to the object being posted to. If there are no object posted to
*                           then the caller must pass a NULL pointer.
*
*               p_tcb       Pointer to the OS_TCB that will receive the 'post'.
*
*               p_void      If we are posting a message to a task, this is the message that the task
*                           will receive.
*
...
*                                           OS_RdyListInit()
*
* Description : This function is called by OSInit() to initialize the ready list. The ready list contains
...
*
* Argument(s) : p_tcb   Pointer to the TCB to insert into the ready list.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*                                         +------------+     +------------+      +------------+
*
* Argument(s) : p_list  Pointer to the OS_RDY_LIST where the OS_TCB will be moved.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*                                          OS_RdyListRemove()
*
* Description : This function is called to remove an OS_TCB from an OS_RDY_LIST knowing the address of the
*               OS_TCB to remove.
*
*               CASE 0: TCB list is empty, nothing to do.
*
...
*                                         +------------+     +------------+      +------------+
*
* Argument(s) : p_tcb   Pointer to the OS_TCB to remove.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
...
*                                      OS_SchedLockTimeMeasStart()
*
* Description : Start measuring the time the scheduler is locked.
*
* Argument(s) : None.
*
* Return(s)   : None.
...
* Note(s)     : (1) This function is INTERNAL to the Kernel and your application MUST NOT call it.
*
*               (2) It's assumed that this function is called when interrupts are disabled.
*
*               (3) We are reading the CPU_TS_TmrRd() directly even if this is a 16-bit timer. The reason
...
*                                      OS_SchedLockTimeMeasStop()
*
* Description : Stop measuring the time the scheduler is locked and update the current and max locked
*               times.
*
* Argument(s) : None.
*
...
*                                         OS_SchedRoundRobin()
*
* Description : This function is called on every tick to determine if a new task at the same priority
*               needs to execute.
*
* Argument(s) : p_rdy_list  Pointer to the OS_RDY_LIST entry of the ready list at the current
*                           priority.
*
* Return(s)   : None.
*
...
*                                            OS_TaskBlock()
*
* Description : This function is called to remove a task from the ready list and also insert it in the
*               timer tick list if the specified timeout is non-zero.
*
* Argument(s) : p_tcb       Pointer to the OS_TCB of the task block.
*
