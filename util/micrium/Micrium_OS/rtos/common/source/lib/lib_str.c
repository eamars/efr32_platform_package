...
#include  <rtos/common/source/lib/lib_str_priv.h>
                                                                /* See Note #1.                                         */
#include  <stdarg.h>
...
*
* Argument(s) : p_str   Pointer to the string (see Note #1).
*
* Return(s)   : Length of a string; number of characters in a string before the terminating NULL
*               character (see Note #2b1).
*
...
*                       (1) String length calculated for string up to but NOT including the NULL
...
*                                              Str_Copy()
*
* Description : Copies the source string to destination string buffer.
*
* Argument(s) : p_str_dest  Pointer to the destination string buffer to receive source string copy (see
*                           Note #1a).
*
*               p_str_src   Pointer to the source string to copy into destination string buffer (see Note
*                           #1b).
*
* Return(s)   : Pointer to the destination string, if NO error(s) [see Note #2b1].
*
*               Pointer to NULL, otherwise (see Note #2b2A).
*
* Note(s)     : (1) (a) Destination buffer size is NOT validated; buffer overruns MUST be prevented by caller.
*
...
*
*                       (1) "The strcpy() function shall copy the string pointed to by 's2' ('p_str_src')
*                            ... into the array pointed to by 's1' ('p_str_dest')" ...
...
*                                             Str_Copy_N()
*
* Description : Copies the source string to the destination string buffer, up to a maximum number of
*               characters.
*
* Argument(s) : p_str_dest  Pointer to the destination string buffer to receive the source string copy
*                           (see Note #1a).
*
*               p_str_src   Pointer to the source string to copy into the destination string buffer (see
*                           Note #1b).
*
*               len_max     Maximum number of characters to copy (see Notes #2a2 and #3d).
*
...
*                               ('p_str_src') to the array pointed to by 's1' ('p_str_dest')";
...
*                              "if the array pointed to by 's2' ('p_str_src') is a string that is shorter
*                               than 'n' ('len_max') bytes, null bytes shall be appended to the copy in
*                               the array pointed to by 's1' ('p_str_dest'), until 'n' ('len_max') bytes
*                               in all are written."
*
...
*                               the array pointed to by 's2' ('p_str_src'), the result is not null-terminated".
*
                              ...
                              CPU_SIZE_T   len_max)
{
    ...
    while (( p_str_copy_dest != DEF_NULL)         &&            /* Copy str until NULL ptr(s)  [see Note #3b]  ...      */
           ( p_str_copy_src  != DEF_NULL)         &&
           (*p_str_copy_src  != (CPU_CHAR  )'\0') &&            /* ... or NULL char found      (see Note #3c); ...      */
           ( len_copy        <  len_max)) {                     /* ... or max nbr chars copied (see Note #3d).          */
    ...
    if (len_copy < len_max) {                                   /* If  copy str len < max buf len (see Note #2a2A), ... */
       *p_str_copy_dest = (CPU_CHAR)'\0';                       /* ... copy NULL char  (see Note #3c1).                 */
    }


    return (p_str_dest);                                        /* Rtn ptr to dest str (see Note #2b1).                 */
}
...
*                                               Str_Cat()
*
* Description : Appends the concatenation string to the destination string.
*
* Argument(s) : p_str_dest  Pointer to the destination string to append concatenation string (see Note
*                           #1a).
*
*               p_str_cat   Pointer to the concatenation string to append to destination string (see Note
*                           #1b).
*
* Return(s)   : Pointer to the destination string, if NO error(s) [see Note #2b1].
*
...
*
*                       (2) (A) "The initial byte of 's2' ('p_str_cat') overwrites the null byte at the
*                                end of 's1' ('p_str_dest')."
...
*                                              Str_Cat_N()
*
* Description : Appends concatenation string to destination string, up to a maximum number of characters.
*
* Argument(s) : p_str_dest  Pointer to the destination   string to append concatenation  string (see Note
*                           #1a).
*
...
*                           #1b).
*
*               len_max     Maximum number of characters to concatenate (see Notes #2a1B and #3d).
*
* Return(s)   : Pointer to the destination string, if NO error(s) [see Note #2b1].
*
*               Pointer to NULL, otherwise (see Note #2b2A).
                             ...
                             CPU_SIZE_T   len_max)
{
...
#if 0
    if (p_str_cat_dest == DEF_NULL) {                           /* Rtn NULL if NULL ptr found (see Note #3b1).          */
        return (DEF_NULL);
    }
#endif
    ...
    while (( p_str_cat_dest != DEF_NULL)         &&             /* Cat str until NULL ptr(s)  [see Note #3b]  ...       */
           ( p_str_cat_src  != DEF_NULL)         &&
           (*p_str_cat_src  != (CPU_CHAR  )'\0') &&             /* ... or NULL char found     (see Note #3c); ...       */
   ...
   *p_str_cat_dest = (CPU_CHAR)'\0';                            /* Append NULL char    (see Note #2a2C).                */
...
*                                               Str_Cmp()
*
* Description : Determines if two strings are identical.
*
* Argument(s) : p1_str  Pointer to the first  string (see Note #1).
*
*               p2_str  Pointer to the second string (see Note #1).
*
* Return(s)   : 0, if strings are identical (see Notes #3a1A, #3a2A, and #3b).
*
...
*               See also Note #2b.
*
* Note(s)     : (1) String buffers are NOT modified.
*
...
*
*                   (a) (1) (A) BOTH string pointer(s) are passed as NULL pointers.
...
*                       (2) Only one NULL character test required in conditional since the previous condition
...
*                                              Str_Cmp_N()
*
* Description : Determines if two strings are identical for up to a maximum number of characters.
*
* Argument(s) : p1_str      Pointer to the first string (see Note #1).
*
*               p2_str      Pointer to the second string (see Note #1).
...
*
* Return(s)   : 0, if strings are identical (see Notes #3a1A, #3a2A, #3b, and #3d).
*
*               Negative value, if 'p1_str' is less than 'p2_str' (see Notes #3a1B1, #3a2B1, and #3c).
*
*               Positive value, if 'p1_str' is greater than 'p2_str' (see Notes #3a1B2, #3a2B2, and #3c).
                              ...
                              CPU_SIZE_T   len_max)
{
    ...
    while ((*p1_str_cmp      == *p2_str_cmp)      &&            /* Cmp strs until non-matching chars (see Note #3c) ... */
           ...
           ( p1_str_cmp_next != DEF_NULL)         &&            /* ... or NULL ptr(s) found          (see Note #3a2).   */
...
*                                          Str_CmpIgnoreCase()
*
* Description : Determines if two strings are identical, ignoring case.
*
* Argument(s) : p1_str  Pointer to the first  string (see Note #1).
*
*               p2_str  Pointer to the second string (see Note #1).
...
*                                              Str_Char()
*
* Description : Searches the string for first occurrence of specific character.
*
* Argument(s) : p_str       Pointer to the string (see Note #1).
*
*               srch_char   Search character.
*
* Return(s)   : Pointer to the first occurrence of search character in string, if any occur (see Note #2b1).
*
...
*                            ... in the string pointed to by 's' ('p_str')."
...
*                                             Str_Char_N()
*
* Description : Searches the string for first occurrence of specific character, up to a maximum number of
*               characters.
*
* Argument(s) : p_str       Pointer to the string (see Note #1).
*
*               len_max     Maximum number of characters to search (see Notes #2c and #3e).
*
*               srch_char   Search character.
*
* Return(s)   : Pointer to the first occurrence of search character in string, if any occur (see Note #2b1).
*
*               Pointer to the NULL, otherwise (see Note #2b2).
*
* Note(s)     : (1) String buffer NOT modified.
*
*               (2) (a) IEEE Std 1003.1, 2004 Edition, Section 'strchr() : DESCRIPTION' states that :
...
*                           of characters; NULL pointer is returned.
                              ...
                              CPU_CHAR     srch_char)
{
    ...
    if (p_str == DEF_NULL) {                                    /* Rtn NULL if srch str ptr NULL (see Note #3a1).       */
        return (DEF_NULL);
    }
...
#if 0
    if (p_str_char == DEF_NULL) {                               /* Rtn NULL if NULL ptr found      (see Note #3b1).     */
        return (DEF_NULL);
    }
#endif

    if (len_srch >= len_max) {                                  /* Rtn NULL if srch char NOT found ...                  */
        return (DEF_NULL);                                      /* ... within max nbr of chars     (see Note #3e1).     */
    }

    if (*p_str_char != srch_char) {                             /* Rtn NULL if srch char NOT found (see Note #3c1).     */
         return (DEF_NULL);
    }


    return ((CPU_CHAR *)p_str_char);                            /* Else rtn ptr to found srch char (see Note #3d1).     */
}
...
*                                            Str_Char_Last()
*
* Description : Searches the string for last occurrence of specific character.
*
* Argument(s) : p_str       Pointer to the string (see Note #1).
*
*               srch_char   Search character.
*
* Return(s)   : Pointer to the last occurrence of search character in string, if any (see Note #2b1).
*
*               Pointer to NULL, otherwise (see Note #2b2).
*
* Note(s)     : (1) String buffer NOT modified.
...
*                                           Str_Char_Last_N()
*
* Description : Searches the string for last occurrence of specific character, up to a maximum number of
*               characters.
*
* Argument(s) : p_str       Pointer to the string (see Note #1).
*
...
*                                          Str_Char_Replace()
*
* Description : Searches the string for specific character and replace it by another specific character.
*
* Argument(s) : p_str           Pointer to the string (see Note #1).
*
...
*
* Return(s)   : Pointer to the string, if NO error(s).
*
                               ...
                               CPU_SIZE_T   len_max)
{
    ...
    if (p_str == DEF_NULL) {                                    /* Rtn NULL if srch str ptr NULL (see Note #2a1).       */
        return (DEF_NULL);
    }
...
*                                               Str_Str()
*
* Description : Searches a string for the first occurrence of a specific search string.
*
* Argument(s) : p_str       Pointer to a string (see Note #1).
*
*               p_str_srch  Pointer to the search string (see Note #1).
*
* Return(s)   : Pointer to the first occurrence of search string in the string, if any (see Note #2b1A).
*
...
*                            pointed to by 's1' ('p_str') of the sequence of bytes ... in the string
*                            pointed to by 's2' ('p_str_srch')"
...
*                                   any other reason(s), NULL is also returned for any error(s).
*
*                       (2) "If 's2' ('p_str_srch') points to a string with zero length, the function
*                            shall return 's1' ('p_str')."
*
*               (3) String search terminates when :
*
...
*                                              Str_Str_N()
*
* Description : Searches the string for the first occurrence of a specific search string, up to a maximum
...
*
* Return(s)   : Pointer to the first occurrence of search string in string, if any occur (see Note #2b1A).
*
                             ...
                             CPU_SIZE_T   len_max)
{
    ...
    return ((CPU_CHAR *)p_str_srch_ix);                         /* Else rtn ptr to found srch str (see Note #3f1).      */
}
...
*                                          Str_FmtNbr_Int32U()
*
* Description : Formats a 32-bit unsigned integer into a multi-digit character string.
*
...
*
*               p_str       Pointer to the character array to return formatted number string (see Note #5).
*
* Return(s)   : Pointer to the formatted string, if NO error(s).
*
*               Pointer to NULL, otherwise.
*
...
*                       array formatting.
*
*                   (b) WARNING: Unless 'p_str' character array is pre-/post-terminated, NULL-character
...
*                                          Str_FmtNbr_Int32S()
*
* Description : Formats a 32-bit signed integer into a multi-digit character string.
*
* Argument(s) : nbr         Number to format.
*
*               nbr_dig     Number of digits to format (see Note #1).
...
*                                            Str_FmtNbr_32()
*
* Description : Formats a number into a multi-digit character string.
*
...
*               p_str       Pointer to the character array to return formatted number string (see Note #5).
*
* Return(s)   : Pointer to the formatted string, if NO error(s) [see Note #6c].
*
*               Pointer to NULL, otherwise.
*
...
*                           integer digits is zero, and the number of digits to format ('nbr_dig') is one
...
*                               integer digits is zero, and the number of digits to format ('nbr_dig') is
...
*                               but fixed in the string as the second character.
*
*                                   Example :
*
*                                       nbr     =  0.7895
...
*                                       p_str     = "##-23456.78"
*
*                   (c) (1) If the integer value of the number to format is zero and
                          ...
                          CPU_CHAR     *p_str)
{
    ...
    CPU_BOOLEAN   nbr_neg_fmtd   = DEF_NO;
                                                                ...
                                                                /* ---------------- VALIDATE FMT ARGS ----------------- */
    if (p_str == DEF_NULL) {                                    /* Rtn NULL if str ptr NULL (see Note #6a).             */
        return (DEF_NULL);
    }
            ...
            if (nbr_dig_sig < LIB_STR_CFG_FP_MAX_NBR_DIG_SIG) { /* If nbr sig digs < max, fmt str digs;           ...   */
                ...
                if ((nbr_shiftd > 0) ||                         /* If shifted nbr > 0                          ...      */
                    ...
                    if ((nbr_neg      == DEF_YES) &&            /* If  nbr neg                     ...                  */
                       ...
                       *p_str_fmt++   = '-';                    /* ... prepend neg sign (see Notes #2b and #3b).        */
                    ...
                    if (nbr_shiftd > 0) {                       /* If shifted nbr > 0,        ...                       */
                       ...
                       *p_str_fmt++ = '0';                      /* ... fmt one '0' char (see Note #3c5).                */
                    }
                   ...
                   *p_str_fmt++   = '-';                        /* ... prepend neg sign (see Note #3b);        ...      */
                    nbr_neg_fmtd = DEF_YES;
    ...
    return (p_str);                                             /* Rtn ptr to fmt'd str (see Note #6e).                 */
}
#endif
...
*                                         Str_ParseNbr_Int32U()
*
* Description : Parses a 32-bit unsigned integer from a string.
*
* Argument(s) : p_str       Pointer to the string (see Notes #1 and #2a).
*
*               p_str_next  Optional pointer to a variable to :
*
*                               (a) Return a pointer to first character following the integer string,
*                                       if NO error(s) (see Note #2a2B2);
*                               (b) Return a pointer to 'p_str',
*                                       otherwise (see Note #2a2A2).
*
*               nbr_base    Base of number to parse (see Notes #2a1B1 and #2a2B1).
*
...
*                       functions shall convert the initial portion of the string pointed to by 'str' ('p_str')
...
*                               (2) "the value of 'str' ('p_str') [is] stored in the object pointed to by 'endptr'
*                                    ('p_str_next'), provided that 'endptr' ('p_str_next') is not a null pointer."
*
...
*                                         Str_ParseNbr_Int32S()
*
* Description : Parses a 32-bit signed integer from a string.
*
* Argument(s) : p_str       Pointer to the string (see Notes #1 and #2a).
*
*               p_str_next  Optional pointer to a variable to ... :
...
*                                             Str_Printf()
*
* Description : printf()-like function that accepts a character output callback.
*
* Argument(s) : out_cb          Character output callback.
*
*               p_out_cb_arg    Character output callback argument.
*
*               format          Format string.
*
*               ...             Variable number of arguments that will be formatted according to the
...
*                                           Str_PrintfImpl()
*
* Description : Generic printf()-like function implementation.
*
* Argument(s) : out_cb          Character output callback (putchar()-like with an extra callback-specific
*                               argument).
*
*               p_out_cb_arg    Character output callback argument.
*
*               format          Format string.
*
*               argp            Variable length argument list.
*
...
*                                            Str_SprintfCb()
*
* Description : Str_Sprintf() and Str_Snprintf() output callback.
*
* Argument(s) : c       Character to be written.
*
*               p_arg   Pointer to an STR_SPRINTF_CB_ARG structure.
*
* Return(s)   : The written character.
*
* Note(s)     : none.
*********************************************************************************************************
*/
...
*                                            Str_FmtOutput()
*
* Description : Justifies and outputs a given string.
*
* Argument(s) : p_str           Pointer to the string to output.
*
*               min_char_cnt    Minimum character count to output (pad if string shorter).
*
*               max_char_cnt    Maximum character count to output (truncate if string longer).
*
*               left_justify    Indicates whether to left or right justify.
*
*               out_cb          Character output callback.
*
*               p_out_cb_arg    Character output callback argument.
*
*               rem_size        Remaining character count to process.
*
* Return(s)   : None
*
* Note(s)     : None.
...
*
* Description : Formats and outputs an unsigned integer.
*
* Argument(s) : nbr             Number to output as a string.
*
*               base            Base used for the string conversion.
*
*               min_digit_cnt   Minimum number of digits to output (pad if less digits).
*
*               left_justify    Indicates whether to left or right justify.
*
*               pad_char        Character used for padding (normally either ' ' or '0').
*
*               out_cb          Character output callback.
*
*               p_out_cb_arg    Character output callback argument.
...
*                                         Str_SngIntFmtOutput()
*
* Description : Formats and outputs an signed integer.
*
* Argument(s) : nbr             Number to output as a string.
*
*               min_digit_cnt   Minimum number of digits to output (pad if less digits).
...
*               pad_char        Character used for padding (normally either ' ' or '0').
*
*               out_cb          putchar-like callback.
*
*               p_out_cb_arg    Argument passed to callback.
*
*               rem_size        Remaining character count to process.
*
* Return(s)   : None.
...
*                                          Str_FmtNbr_Int32()
*
* Description : Formats a 32-bit integer into a multi-digit character string.
*
* Argument(s) : nbr         Number to format.
*
*               nbr_dig     Number of digits to format (see Note #1).
...
*               p_str       Pointer to the character array to return formatted number string (see Note #5).
*
* Return(s)   : Pointer to the formatted string, if NO error(s) [see Note #6f].
*
*               Pointer to NULL, otherwise.
*
...
*                       (1) Invalid string format performed; NULL pointer is returned.
*
                                     ...
                                     CPU_CHAR     *p_str)
{
    ...
    return (p_str);                                             /* Rtn ptr to fmt'd str (see Note #6f).                 */
}
...
*                                         Str_ParseNbr_Int32()
*
* Description : Parses a 32-bit integer from a string.
*
* Argument(s) : p_str       Pointer to the string (see Notes #1 and #2a).
*
*               p_str_next  Optional pointer to a variable to :
...
*
*               p_nbr_neg   Pointer to the variable to return if the parsed (signed) number is negative :
*
*                               DEF_NO                  Number is non-negative.
*                               DEF_YES                 Number is negative.
*
...
*                           (B) 'p_str_next' points to invalid character.
*
*                       (3) Entire  parse string converted (see Note #2a2B).
*                           (A) Parsed integer returned.
*                           (B) 'p_str_next' points to terminating NULL character.
*
                                               ...
                                               CPU_BOOLEAN   *p_nbr_neg)
{
   ...
   *p_str_next = (CPU_CHAR *)p_str;                             /* Init rtn str for err (see Note #5).                  */
   ...
   *p_nbr_neg = DEF_NO;                                         /* Init nbr neg for err (see Note #5).                  */


    if (p_str == DEF_NULL) {                                    /* Rtn zero if str ptr NULL (see Note #4b1).            */
        return (0u);
    }
    ...
    p_str_parse = p_str;                                        /* Save ptr to init'l str for err (see Note #2a2A2).    */
    ...
    p_str_parse_nbr = p_str_parse;                              /* Save ptr to str's nbr (see Note #2a1A1).             */

    switch (nbr_base) {
        case  0u:                                               /* Determine unspecified nbr base (see Notes #2a1B1a).  */
             if (*p_str_parse == '0') {                         /* If avail, ...                                        */
                  p_str_parse++;                                /* ... adv past '0'  prefix (see Note #2a1B1b2).        */
                  switch (*p_str_parse) {
                      ...
                      case 'X':
                           ...
                           if (nbr_hex == DEF_YES) {            /* If next char is valid hex dig, ...                   */
                               p_str_parse++;                   /* ... adv past '0x' prefix (see Note #2a1B1b2A).       */
                           }
                           break;
        ...
        case  8u:                                               /* See Note #2a1B1a2.                                   */
             if (*p_str_parse == '0') {                         /* If avail, ...                                        */
                  p_str_parse++;                                /* ... adv past '0'  prefix (see Note #2a1B1b2B).       */
             }
             break;
    ...
    if (p_str_parse != p_str_parse_nbr) {                       /* If final parse str != init'l parse nbr str,       .. */
       *p_str_next = (CPU_CHAR *)p_str_parse;                   /* .. rtn   parse str's next char (see Note #2a2B2); .. */
    } else {
       *p_str_next = (CPU_CHAR *)p_str;                         /* .. else rtn initial parse str  (see Note #2a2A2).    */
    }
