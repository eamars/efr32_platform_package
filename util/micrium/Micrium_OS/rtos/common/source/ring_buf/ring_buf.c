...
#include  <rtos/common/source/ring_buf/ring_buf_priv.h>
...
*                                            RingBufCreate()
*
* Description : Create and initializes a ring buffer.
*
* Argument(s) : p_ring_buf  Pointer to ring buffer structure to use.
*
*               p_mem_seg   Pointer to memory segment to use to allocate buffer. If DEF_NULL, the LIB Mem
*                           heap will be used.
*
*               buf_size    Size of the ring buffer, in bytes.
*
...
* Return(s)   : none.
*
* Note(s)     : (1) Buffer size must be at least 128 bytes and less than 32767 bytes.
*********************************************************************************************************
*/
...
*                                           RingBufWrAlloc()
*
* Description : Allocate space to write data in the ring buffer.
*
* Argument(s) : p_ring_buf  Pointer to ring buffer structure to use.
*
*               size        Size to allocate, in bytes. MUST be greater than 0.
*
* Return(s)   : Pointer to buffer location where to write, if NO error(s),
*               DEF_NULL,                                  otherwise.
*
* Note(s)     : (1) This function MUST be called from a CRITICAL SECTION, the same in which the call to
...
*                                           RingBufWrCommit()
*
* Description : Commit write data to ring buffer. After this call, no other writes should be done, unless
...
*                                              RingBufWr()
*
* Description : Wrapper function that calls both RingBufWrAlloc() and RingBufWrCommit(), in a critical
*               section.
*
* Argument(s) : p_ring_buf      Pointer to ring buffer structure to use.
*
*               size            Size of the data to write in the ring buffer, in bytes.
*
*               p_start_data    Pointer to start of data to write in the ring buffer.
*
* Return(s)   : DEF_OK    if write was successful,
*               DEF_FAIL, otherwise.
*
* Note(s)     : none.
*********************************************************************************************************
...
*                                         RingBufDataIsAvail()
*
* Description : See if there is any data that is available, in the ring buffer.
*
* Argument(s) : p_ring_buf  Pointer to ring buffer structure to use.
*
* Return(s)   : DEF_YES,  if there is data that is available to read in the ring buffer,
*               DEF_FAIL, otherwise.
*
* Note(s)     : none.
*********************************************************************************************************
...
*                                           RingBufRdStart()
*
* Description : Start reading from the next available read location, in the ring buffer.
*
* Argument(s) : p_ring_buf  Pointer to ring buffer structure to use.
*
* Return(s)   : DEF_OK    if there is data to read in the ring buffer,
*               DEF_FAIL, otherwise.
*
* Note(s)     : (1) This function must be called before any call to RingBufRd() is made. Once finished
...
*                                              RingBufRd()
*
* Description : Read from the next available read location, in the ring buffer.
*
* Argument(s) : p_ring_buf  Pointer to ring buffer structure to use.
*
*               size        Size read from the buffer.
*
* Return(s)   : Pointer to buffer where to read, if NO error(s),
*               DEF_NULL,                        otherwise.
*
* Note(s)     : (1) RingBufRdStart() must be called before making any call to this function. This
...
*                                            RingBufRdEnd()
*
* Description : Finish reading from the ring buffer.
*
* Argument(s) : p_ring_buf  Pointer to ring buffer structure to use.
*
* Return(s)   : none.
*
* Note(s)     : (1) This function must be called after the call(s) to RingBufRd() is/are made and a
...
*
* Description : Calculates and updates the RdIxNext if the rd location had been overwritten by a write.
*
* Argument(s) : p_ring_buf  Pointer to ring buffer structure to use.
*
* Return(s)   : none.
*
* Note(s)     : (1) This function MUST be called from a CRITICAL SECTION, the same in which the
