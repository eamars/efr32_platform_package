...
typedef  struct  shell_data {
    ...
    CPU_CHAR          *SharedBufPtr;                            /* Ptr to start of shared buf.                          */
                                                                /* This field need to be protected by a crit section.   */
    CPU_BOOLEAN        SharedBufIsUsed;                         /* Flag indicating if shared buf is in use.             */
} SHELL_DATA;
...
*
* Description : Configure the properties of the commands used by the Shell sub-module.
*
* Argument(s) : p_cfg   Pointer to the structure containing the command usage parameters.
*
* Return(s)   : None.
*
* Note(s)     : (1) This function is optional. If it is called, it must be called before Shell_Init(). If
...
*
* Description : Configure the memory segment that will be used to allocate internal data needed by Shell
...
*                                             Shell_Init()
*
* Description : Initializes the shell module.
*
...
* Return(s)   : None.
*
* Note(s)     : (1) Shell_Init() MUST be called before the other Shell functions are invoked, except for
*                   the 'Configure' functions.
*
*               (2) Shell_Init() MUST ONLY be called ONCE from the product's application.
*
*               (3) Module command pools MUST be initialized before initializing the pool with pointers
*                   to module commands.
*
*               (4) The functions Shell_Configure...() can be used to configure more specific properties
...
*                                             Shell_Exec()
*
* Description : Parses and executes a command passed in parameter. :
*
* Argument(s) : in              Pointer to a CPU_CHAR string holding a complete command and its
*                               argument(s). This string MUST be editable, it cannot be 'const'.
*
*               out_fnct        Pointer to the 'output' function used by command (see Note #1).
*
*               p_cmd_param     Pointer to the command additional parameters.
*
...
*
* Note(s)     : (1) The command may generate some output that should be transmitted to some devices
                        ...
                        RTOS_ERR         *p_err)
{
    ...
    RTOS_ASSERT_DBG_ERR_SET((in != DEF_NULL), *p_err, RTOS_ERR_NULL_PTR, 0);
...
*                                           Shell_CmdTblAdd()
*
* Description : (1) Allocates and initializes a module command as follows :
*
...
* Return(s)   : None.
*
* Note(s)     : (4) The 'cmd_tbl_name' parameter is not mandatory in the current implementation.
                       ...
                       RTOS_ERR   *p_err)
{
    ...
    if (RTOS_ERR_CODE_GET(*p_err) != RTOS_ERR_NONE) {
        ...
        return;
                                                                ...
                                                                /* If cmd tbl name not gotten from param ...            */
    if (p_tbl_name == DEF_NULL) {                               /* ... get name from first cmd.                         */
...
*                                           Shell_CmdTblRem()
*
* Description : Removes a module command.
*
* Argument(s) : cmd_tbl_name    Pointer to character string representing the name of the command table.
*
*               p_err           Pointer to the variable that will receive one of the following error
...
*                                            Shell_Scanner()
*
* Description : Scans and parses the command line.
*
* Argument(s) : in              Pointer to a NULL terminated string holding a complete command and its
*                               argument(s).
*
...
*                                           Shell_OptParse()
*
* Description : Parses optional command line arguments.
*
* Argument(s) : argc        Argument count.
*
*               argv        Pointer to argument table.
*
*               state       Argument parsing state structure.
*
*               opt_str     Argument string to parse.
*
* Return(s)   : SHELL_OPTPARSE_STATE.
*
* Note(s)     : None.
*********************************************************************************************************
*/
...
*                                         Shell_ListCmdOutput()
*
* Description : Outputs a list of all commands.
*
...
* Return(s)   : None.
*
* Note(s)     : (1) This function must only be called with the Shell lock acquired.
*********************************************************************************************************
*/
...
*                                         Shell_SharedBufGet()
*
* Description : Obtain temporary access to shared buffer offered by shell. The maximum size is set to
*               CmdNameLenMax in the shell cfg passed at initialization.
*
* Argument(s) : req_len     Requested length for the buffer. Must be smaller or equal to CmdNameLenMax.
*
...
*                               RTOS_ERR_OWNERSHIP
*
* Return(s)   : Pointer to shared buffer start, if NO error(s),
*               DEF_NULL, otherwise.
*
* Note(s)     : None.
*********************************************************************************************************
...
*                                         Shell_SharedBufFree()
*
* Description : Release ownership on temporary buffer.
*
* Argument(s) : None.
*
* Return(s)   : None.
...
*                                           Shell_CmdSearch()
*
* Description : Search for specified command in command tables.
*
* Argument(s) : cmd_name    Pointer to command name.
*
* Return(s)   : Pointer to command function if found,
*               DEF_NULL, otherwise.
*
* Note(s)     : (1) Lock must be acquired when calling this function.
*********************************************************************************************************
*/
...
*
* Return(s)   : DEF_OK    if name has been correctly obtained.
*               DEF_FAIL, otherwise.
*
* Note(s)     : (1) The command module name consists in the first part of a command part, that is the
...
*                                    Shell_ModuleCmdAllocCallback()
*
* Description : Callback function executed when a SHELL_MODULE_CMD block is obtained from pool for the
*               first time.
*
...
*
*               p_blk   Pointer to allocated block.
*
*               p_arg   Pointer to arg, nothing in this case.
*
