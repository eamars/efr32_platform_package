...
#include  <rtos/common/source/op_lock/op_lock_priv.h>
...
*                                            OpLockCreate()
*
* Description : Initialize an operation lock.
*
* Argument(s) : p_op_lock   Pointer to operation lock structure to initialize.
*
...
*                                            OpLockAcquire()
*
* Description : Acquire an operation lock for a given operation.
*
* Argument(s) : p_op_lock   Pointer to operation lock structure.
*
*               p_op_desc   Pointer to operation lock operation description structure.
*
*               timeout_ms  TImeout, in milliseconds, to pend if lock is not available.
*
*               p_err       Pointer to variable that will receive the return error code from this function :
*
*                               RTOS_ERR_NONE
...
*                                            OpLockRelease()
*
* Description : Release an operation lock for a given operation.
*
* Argument(s) : p_op_lock   Pointer to operation lock structure.
*
*               p_op_desc   Pointer to operation lock operation description structure. It should be
*                           the same as for the call to Common_OpLockAcquire().
*
* Return(s)   : none.
*
* Note(s)     : (1) This variable is used throughout the evaluation function to accumulate pending
...
*                                              OpLockDel()
*
* Description : Delete/free an operation lock.
*
* Argument(s) : p_op_lock   Pointer to operation lock structure to delete/free.
*
* Return(s)   : DEF_OK,   if lock was deleted/freed correctly,
*
*               DEF_FAIL, otherwise.
*
* Note(s)     : none.
...
*                                             OpLockEnter()
*
* Description : On enter callback when acquiring an operation lock.
*
* Argument(s) : p_mon_data  Pointer to data passed to KAL_MonCreate(). In this case, a pointer to the
*                           OP_LOCK structure used.
*
*               p_data      Pointer to data passed to KAL_MonOp(). In this case, the
*                           OP_LOCK_DATA on the caller's stack.
*
* Return(s)   : KAL_MON_RES_BLOCK,     if task must     be blocked,
*
*               KAL_MON_RES_STOP_EVAL, if task must not be blocked and the evaluations must be stopped.
*
* Note(s)     : (1) If any operation is currently holding op lock, operation must be initially blocked,
...
*                                             OpLockExit()
*
* Description : On enter callback when releasing an operation lock.
*
* Argument(s) : p_mon_data  Pointer to data passed to KAL_MonCreate(). In this case, a pointer to the
*                           OP_LOCK structure used.
*
...
*                           OP_LOCK_DATA on the caller's stack.
*
* Return(s)   : KAL_MON_RES_ALLOW,     if the task must run and the evaluation must be continued.
*
*               KAL_MON_RES_STOP_EVAL, if the task must run and the evaluation must be stopped.
*
* Note(s)     : (1) No need to reevaluate predicates if the operation count is not zero (i.e. no
...
*                                             OpLockEval()
*
* Description : On evaluation callback when evaluation if an operation lock can be allowed to execute.
*
* Argument(s) : p_mon_data      Pointer to data passed to KAL_MonCreate(). In this case, a pointer to the
*                               OP_LOCK structure used.
*
*               p_eval_op_data  Pointer to data passed to KAL_MonOp(). In this case, the
*                               OP_LOCK_DATA on one of the previous caller's stack.
*
*               p_scan_op_data  Pointer to data passed to KAL_MonOp(). In this case, the
*                               OP_LOCK_DATA on the caller's stack.
*
* Return(s)   : A combination of these flags:
*
