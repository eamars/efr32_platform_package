...
#include  <rtos/cpu/include/cpu_port_sel.h>
...
#error "Extended errors cannot be enabled when using legacy mode for errors."
#endif
#endif
...
*
* Description : Obtains the error string or the error description string, respectively, from an error
*               code. Will return a string saying strings are unavailable if RTOS_ERR_CFG_STR_EN is set
*               to DEF_DISABLED.
*
* Argument(s) : err_code    Error code for which to obtain the string.
*
* Return(s)   : The required string (either the error or the description).
*
* Note(s)     : None.
*********************************************************************************************************
*/
...
*                                          RTOS_ERR_CODE_GET()
*
* Description : Obtains the error code from an error variable, no matter how RTOS_ERR_CFG_LEGACY_EN is
*               configured.
*
* Argument(s) : err_var     Error variable from which to obtain error code.
*
* Return(s)   : Error code in passed variable.
*
* Note(s)     : None.
*********************************************************************************************************
*/
...
#ifndef  RTOS_ERR_STR_MODULE                                    /* If file is used to generate err strings. See Note #2.*/
#if (RTOS_ERR_CFG_LEGACY_EN == DEF_ENABLED)
    #define  RTOS_ERR_CODE_GET(err_var)                     (err_var)
#else
...
*                                            RTOS_ERR_SET()
*
* Description : Sets a given error variable to an error value. Also sets the extended error, if enabled.
*
* Argument(s) : err_var     Error variable to set to 'err'.
*
*               err_code    Error value to which to set 'err_var'.
*
* Return(s)   : None.
*
* Note(s)     : None.
...
#ifndef  RTOS_ERR_STR_MODULE                                    /* If file is used to generate err strings. See Note #2.*/
#if (RTOS_ERR_CFG_LEGACY_EN == DEF_ENABLED)
    #define  RTOS_ERR_SET(err_var, err_code)                    (err_var) = (err_code)
#else
...
*                                            RTOS_ERR_COPY()
*
* Description : Copies every field of an error variable to another one.
*
* Argument(s) : err_dst     Destination error variable.
*
*               err_src     Source error variable.
*
* Return(s)   : None.
*
* Note(s)     : (1) This wrapper is present if other fields require particular manipulations to copy,
...
*                                               ERR CODES
*********************************************************************************************************
* Note(s) : (1) User should NEVER rely on the order/numerical value of ANY of these enum items. Their
...
*                                             EXTENDED ERR
*********************************************************************************************************
* Note(s) : (1) The RTOS_ERR_CFG_LEGACY_EN is provided to allow usage of existing code without needing to
*               modify anything. New applications should disable it, to enable usage of extended errors
*               and other advantages such as unified error codes.
*********************************************************************************************************
*/

#ifndef  RTOS_ERR_STR_MODULE                                    /* If file is used to generate err strings. See Note #2.*/
#if (RTOS_ERR_CFG_LEGACY_EN == DEF_ENABLED)                     /* If legacy, do not use extended err.                  */
typedef  RTOS_ERR_CODE  RTOS_ERR;                               /* RTOS_ERR is an enum.                                 */
#include  <rtos/common/include/rtos_err_compatibility.h>        /* Include old error types and values.                  */
#else
