...
*                                           MEM_SEG_INIT()
*
* Description : Creates a new memory segment to be used for runtime memory allocation. Created segment
...
* Return(s)   : None.
*
* Note(s)     : (1) Contrary to the Mem_SegCreate() function, this segment is not checked for overlap at
...
{                                                                           \
    /* .AddrBase         = */ (CPU_ADDR) (seg_base_addr),                   \
    /* .AddrEnd          = */ (CPU_ADDR)((seg_base_addr) + ((size) - 1u)),  \
    /* .AddrNext         = */ (CPU_ADDR) (seg_base_addr),                   \
    /* .NextPtr          = */ DEF_NULL,                                     \
...
*
* Note(s) : (1) Callback that is called when a new block is allocated from the memory segment when a
...
*
*           (2) There are 2 different types of dynamic memory pool:
*
...
*
*  Note(s) : (1) The following data types are deprecated. Unless already used, these data types should not
...
typedef  struct  mem_pool {                                     /* --------------------- MEM POOL --------------------- */
...
#error  "CPU_CFG_ENDIAN_TYPE illegally #defined in 'cpu.h'. [See 'cpu_port.h CONFIGURATION ERRORS']"
